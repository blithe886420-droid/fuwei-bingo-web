...
function isFormalHardRejectCandidate(group = {}, slotNo = 1, phaseContext = null) {
  ...
  // ✅ 修正四：三星 hit3 理論1%，50期內 hit3=0 機率約60%，原條件過嚴
  // 改為需要 hit2Rate 也是0才拒絕（三星 hit2 理論14%，兩個都是0才真的很差）
  if (slotNo <= 2 && totalRounds < 50 && blendedHit3Rate <= 0 && toNum(stats.hit2Rate, 0) <= 0) {
    return true;
  }

  // 第1槽：輪數不足 20 輪直接拒絕（原30輪改為20輪，給稍多機會）
  if (slotNo === 1 && totalRounds < 20) {
    return true;
  }
...
function getHit3RateFloor(role = 'mix', selection = {}, phaseContext = null) {
  // ✅ 修正五：三星 hit3 理論值只有 1.06%，attack 槽原門檻 2% 過嚴
  // 調整為更接近理論值的門檻
  let floor = 0.008;
  if (role === 'attack') floor = 0.010;  // 貼近三星理論值1.06%
  if (role === 'extend') floor = 0.007;
  if (role === 'guard') floor = 0.004;
  if (role === 'recent') floor = 0.006;
...
function scoreGroupForMode(group, role = 'mix', strategyMode = 'mix', riskMode = 'balanced', pools = {}, phaseContext = null) {
  ...
  // ✅ 三星修正：移除 hit4_rate 和 recent_50_hit4_rate（三星最多中3，hit4永遠是0）
  // 原本 hit4_rate*5200 + recent_50_hit4_rate*6000 = 理論最高11200但永遠是0
  // 這個空洞讓三星策略評分被壓縮，好壞策略分數差距縮小，改為加重 hit2
  score += toNum(meta.hit2_rate, 0) * 2800;
  score += toNum(meta.recent_50_hit_rate, 0) * 2400;
  ...
  // ✅ 三星修正：移除 phaseBucket.recent20Hit4Rate（三星不可能hit4，永遠是0）
  // 原本 *7600 是整個評分裡最大的單項，卻永遠是0，改為加重 hit2
  score += phaseBucket.hit2Rate * 3600;
...
async function insertThreeStarDerivative(db, formalGroups, sourceDrawNo, latestDraw, phaseContext) {
  try {
    ...
    (statsRows3s?.data || []).forEach(row => {
      ...
      const effectiveHit3Rate = last30Hits.length >= 10 ? last30Hit3Rate * 0.7 + last10Hit3Rate * 0.3 : (rounds > 0 ? Number(row.hit3||0)/rounds : 0);
      const effectiveHit2Rate = last30Hits.length >= 10 ? last30Hit2Rate * 0.7 + last10Hit2Rate * 0.3 : (rounds > 0 ? Number(row.hit2||0)/rounds : 0);
    ...
    // 【方向三】計算近20期號碼頻率，注入 snapshot
    const recentDrawsForFreq3s = (marketRows.data || []).slice(0, 20);
    const numFreqMap3s = new Map();
    for (let n = 1; n <= 80; n++) numFreqMap3s.set(n, 0);
    recentDrawsForFreq3s.forEach(row => {
      const nums = parseNums(row?.numbers || row?.draw_numbers || []);
      nums.forEach(n => numFreqMap3s.set(n, (numFreqMap3s.get(n) || 0) + 1));
    });
    const hotFreqNums3s = [...numFreqMap3s.entries()]
      .filter(([, f]) => f >= 6).sort((a, b) => b[1] - a[1]).map(([n]) => n);
