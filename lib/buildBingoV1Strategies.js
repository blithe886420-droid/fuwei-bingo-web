...
function decideDynamicRoleAllocation(condition = {}, totalGroups = 8) {
  ...
  if (marketPhase === 'continuation') {
    // ✅ 步驟五數據驅動（5/23）：recent(2.3%) > rebound/cold(1.9%) > hot > zone_fill > scatter
    // streak完全移除（dynamic_streak_5命中率0%）
    if (streakStrength >= 5) {
      // 有連號：保留streak但減量，加重recent和cold
      roles = ['recent', 'cold', 'streak', 'recent', 'hot', 'cold', 'zone_fill', 'scatter'];
    } else if (hotConcentration >= 30) {
      // 熱區高度集中：recent+cold為主
      roles = ['recent', 'cold', 'recent', 'cold', 'hot', 'zone_fill', 'scatter', 'hot'];
    } else {
      // 一般輪動：recent+cold均衡
      roles = ['recent', 'cold', 'recent', 'hot', 'cold', 'zone_fill', 'scatter', 'hot'];
    }
