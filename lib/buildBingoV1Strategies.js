...
function getTimeSlotWeights() {
  ...
  if (taipeiHour >= 7 && taipeiHour < 12) {
    // 早上 07:00~12:00：盤面較規律，連號多，熱號加重，回補降低
    return {
      slot: 'morning',
      freqWeight: 2.0,
      hotWeight: { hot5: 10.0, hot10: 5.0, hot20: 2.5 },
      streakWeight: { streak4: 7.0, streak3: 5.0 },
      overdueWeight: { continuation: 0.8, chaos: 3.0, default: 2.0 },
      coldWeight: { continuation: 0.3, default: 1.5 },
      hotFreqWeight: 0.4
    };
  } else if (taipeiHour >= 12 && taipeiHour < 18) {
    // 下午 12:00~18:00：盤面散亂，回補最有效，連號降低
    return {
      slot: 'afternoon',
      freqWeight: 1.5,
      hotWeight: { hot5: 6.0, hot10: 3.0, hot20: 1.5 },
      streakWeight: { streak4: 1.5, streak3: 1.0 },
      overdueWeight: { continuation: 1.2, chaos: 5.0, default: 3.5 },
      coldWeight: { continuation: 0.8, default: 2.5 },
      hotFreqWeight: 0.25
    };
  } else if (taipeiHour >= 18 && taipeiHour < 23) {
    // 晚上 18:00~23:00：介於中間，平衡權重
    return {
      slot: 'evening',
      freqWeight: 1.8,
    ...
    return {
      ...
      freqWeight: 1.5,
      hotWeight: { hot5: 6.0, hot10: 3.0, hot20: 1.5 },
      streakWeight: { streak4: 2.0, streak3: 1.5 },
      overdueWeight: { continuation: 1.0, chaos: 4.5, default: 3.2 },
      coldWeight: { continuation: 0.6, default: 2.2 },
      hotFreqWeight: 0.28
    };
  }
}
    ...
    if (boardState && boardState.groups.length >= totalGroups) {
      ...
      boardState.groups.slice(0, totalGroups).forEach((groupNums, idx) => {
        ...
        rawStrategies.push({
          ...
          nums,
          reason: `盤面最優分配[${role}] top24覆蓋 盤相:${boardState.marketPhase} 時段:${boardState.timeSlot || ''}`,
          meta: {
            model: 'v18_board_driven',
            role,
            board_rank: idx + 1,
            board_meta: boardState.boardMeta || {},
            market_condition: {
