...
function analyzeBoardState(allRows = [], marketSnapshot = {}) {
  ...
  for (let n = 1; n <= 80; n++) {
    ...
    if (gap && gap.overdueScore > 0) {
      const overdueWeight = marketPhase === 'continuation' ? tw.overdueWeight.continuation :
                            marketPhase === 'hot_streak' ? tw.overdueWeight.continuation :  // ✅ hot_streak 跟 continuation 一樣追熱
    ...
    let coldContrib = 0;
    if (coldIdx >= 0) {
      const coldWeight = (marketPhase === 'continuation' || marketPhase === 'hot_streak')
        ? tw.coldWeight.continuation : tw.coldWeight.default;  // ✅ hot_streak 跟 continuation 一樣輕冷號
  ...
  // ✅ v2：區間均衡保護
  // 四個區間（1-20/21-40/41-60/61-80）每區最多6顆，避免任何區間爆滿
  // 1-20小號區額外限制最多5顆，避免小號過密（昨天實測發現低號集中在1-20）
  for (const { n, score } of ranked) {
    ...
    if (zone === 1 && zoneCount >= 5) continue;  // ✅ 1-20區最多5顆
    if (zoneCount >= 6) continue;
    selected.push(n);
  ...
  // ✅ v2：高低號平衡保護（低號1-40，高號41-80）
  // 低號至少8顆（8組每組至少1顆低號），最多14顆（避免過多低號）
  // 高號至少8顆，最多14顆
  const getLow = () => selected.filter(n => n <= 40);
  const getHigh = () => selected.filter(n => n > 40);

  // 補足低號（不足8顆），同時避免1-20區超過5顆
  if (getLow().length < 8) {
    const needed = 8 - getLow().length;
    let added = 0;
    for (const { n } of ranked) {
      if (added >= needed) break;
      if (n <= 40 && !selected.includes(n)) {
        // 1-20區已有5顆時，只補21-40的低號
        const zone1Count = selected.filter(x => x <= 20).length;
        if (n <= 20 && zone1Count >= 5) continue;
        const worstHighIdx = selected.map((x, i) => ({ x, i, s: scores[x] || 0 }))
          .filter(({ x }) => x > 40).sort((a, b) => a.s - b.s)[0];
        if (worstHighIdx) { selected.splice(worstHighIdx.i, 1, n); added++; }
      }
    }
  }
  ...
  // 補足高號（不足8顆）
  if (getHigh().length < 8) {
    const needed = 8 - getHigh().length;
    let added = 0;
    for (const { n } of ranked) {
      if (added >= needed) break;
      if (n > 40 && !selected.includes(n)) {
        const worstLowIdx = selected.map((x, i) => ({ x, i, s: scores[x] || 0 }))
          .filter(({ x }) => x <= 40).sort((a, b) => a.s - b.s)[0];
        if (worstLowIdx) { selected.splice(worstLowIdx.i, 1, n); added++; }
      }
    }
  }

  // 限制低號上限（不超過14顆，避免低號爆滿）
  while (getLow().length > 14) {
    const worstLowIdx = selected.map((x, i) => ({ x, i, s: scores[x] || 0 }))
      .filter(({ x }) => x <= 40).sort((a, b) => a.s - b.s)[0];
    // 找一顆高號補入
    const bestHighNotIn = ranked.find(({ n }) => n > 40 && !selected.includes(n));
    if (worstLowIdx && bestHighNotIn) {
      selected.splice(worstLowIdx.i, 1, bestHighNotIn.n);
    } else break;
  }

  // 限制高號上限（不超過14顆，避免高號爆滿）
  while (getHigh().length > 14) {
    const worstHighIdx = selected.map((x, i) => ({ x, i, s: scores[x] || 0 }))
      .filter(({ x }) => x > 40).sort((a, b) => a.s - b.s)[0];
    const bestLowNotIn = ranked.find(({ n }) => n <= 40 && !selected.includes(n));
    if (worstHighIdx && bestLowNotIn) {
      selected.splice(worstHighIdx.i, 1, bestLowNotIn.n);
    } else break;
  }
...
function distributeToGroups(nums = [], scores = {}, groupCount = 8, numPerGroup = 3) {
  ...
  // ✅ fix：加入高低號平衡邏輯
  // 每組3顆中：至少1顆低號(1-40)、至少1顆高號(41-80)
  // 避免每組都擠在高號區（41-80），導致低號完全沒覆蓋
  const isLow = n => n <= 40;
  const isHigh = n => n > 40;
  ...
  for (const n of sorted) {
    ...
    for (let i = 0; i < groupCount; i++) {
      ...
      let diversity = zones.size * 10 + (numPerGroup - group.length);

      // ✅ 高低號平衡加分
      const lowCount = group.filter(isLow).length;
      const highCount = group.filter(isHigh).length;

      // 這組缺低號且 n 是低號 → 加分
      if (lowCount === 0 && isLow(n)) diversity += 15;
      // 這組缺高號且 n 是高號 → 加分
      if (highCount === 0 && isHigh(n)) diversity += 15;
      // 這組已有2顆高號，再加高號 → 扣分（避免全高號）
      if (highCount >= 2 && isHigh(n)) diversity -= 20;
      // 這組已有2顆低號，再加低號 → 扣分（避免全低號）
      if (lowCount >= 2 && isLow(n)) diversity -= 20;
...
function buildHotChase(todayRows, recent20, historyRows, optimizerWeight, marketProfile, marketSnapshot = {}) {
  ...
  // ✅ 根據盤相動態調整權重
  const todayWeight = (marketPhase === 'continuation' || marketPhase === 'hot_streak') ? 5.5 :
                      marketPhase === 'hot_bias' ? 5.0 :
                      marketPhase === 'chaos' ? 2.5 : 4.0;  // ✅ 新盤相對應權重
  const recentWeight = (marketPhase === 'continuation' || marketPhase === 'hot_streak') ? 2.5 :
                       marketPhase === 'hot_bias' ? 2.3 : 2.0;
...
function buildRebound(todayRows, recent20, historyRows, optimizerWeight, marketProfile, marketSnapshot = {}) {
  ...
  const todayPenalty = (marketPhase === 'continuation' || marketPhase === 'hot_streak' || marketPhase === 'hot_bias') ? 1.4 :
                      0.9 + (2 - optimizerWeight) * 0.35;  // ✅ 熱盤相冷號懲罰更重
  const recentPenalty = (marketPhase === 'continuation' || marketPhase === 'hot_streak' || marketPhase === 'hot_bias') ? 1.0 :
                        0.6 + (2 - optimizerWeight) * 0.25;
...
function buildZoneBalanced(todayRows, recent20, historyRows, optimizerWeight, marketProfile, marketSnapshot = {}, numCount = 4) {
  ...
  if (marketPhase === 'continuation' || marketPhase === 'bias' || marketPhase === 'hot_bias' || marketPhase === 'hot_streak') {  // ✅ 新盤相也追熱區
    hot5.forEach(n => {
      const key = String(n).padStart(2, "0");
      merged[key] = (merged[key] || 0) + 6;
    });
  ...
  const zoneTargets = (marketPhase === 'continuation' || marketPhase === 'bias' ||
                       marketPhase === 'hot_bias' || marketPhase === 'hot_streak')
...
function decideDynamicRoleAllocation(condition = {}, totalGroups = 8, roleWeights = {}) {
  ...
  if (marketPhase === 'continuation') {
    ...
    // ✅ rotation 盤：以步驟五歷史數據為主
    // rebound(cold邏輯) 2.3% > recent 1.9% > zone_fill > hot > scatter
    // 根據盤面細部指標微調比例
    if (coldPressureHigh && !hasStreak) {
      // 冷號壓力大+無連號：cold主攻，recent輔助
      roles = ['cold', 'recent', 'cold', 'recent', 'cold', 'zone_fill', 'hot', 'scatter'];
    } else if (hasStreak) {
      // 有連號：streak+chain為主，cold+recent輔助
      roles = ['streak', 'chain', 'cold', 'recent', 'hot', 'dominant', 'zone_fill', 'scatter'];
    } else if (hotOverlapLow) {
      // 熱號命中率低：cold+scatter廣覆蓋
      roles = ['cold', 'recent', 'cold', 'scatter', 'zone_fill', 'recent', 'hot', 'balance'];
    } else if (hotOverlapHigh) {
      // 熱號高度重疊：repeat+recent鎖定
      roles = ['recent', 'repeat', 'cold', 'recent', 'hot', 'cold', 'zone_fill', 'scatter'];
    } else {
      // 標準輪動：cold+recent為核心
      roles = ['cold', 'recent', 'hot', 'cold', 'recent', 'zone_fill', 'scatter', 'balance'];
    }
  }

  // ✅ v5：根據 roleWeights 動態調整角色出場順序
  // 改用明確對應表，不再用 token 猜測
  // 與 auto-train.js 的 STRATEGY_ROLE_MAP 保持一致
  if (Object.keys(roleWeights).length > 0) {
    const STRATEGY_ROLE_MAP_BUILD = {
      'dynamic_hot_5':       ['hot'],
      'dynamic_hot_6':       ['hot'],
    ...
    const getRoleWeight = (role) => {
      let totalW = 0;
      let count = 0;
      for (const [stratKey, w] of Object.entries(roleWeights)) {
        const roles = STRATEGY_ROLE_MAP_BUILD[stratKey] || [];
        if (roles.includes(role)) {
          totalW += w;
          count++;
        }
      }
      // 該角色有對應策略就用平均權重，否則預設 1.0
      return count > 0 ? totalW / count : 1.0;
    };
...
function buildNumsByRole(role = 'scatter', condition = {}, usedNums = new Set(), numCount = 3, seed = 0, marketSnapshot = {}) {
  ...
  if (role === 'streak') {
    ...
    // ── 強打者 ──
    // 專長：短期+長期熱號結合，優先最近5期的熱號
    // 底層思維：短期狀態比長期重要，越近期資料權重越高
    // hot5（最近5期熱號）權重最高，freq20Hot（近20期）次之
    const shortTermHot = [...hot5, ...hot10.filter(n => !hot5.includes(n))];
    tryAdd([...shortTermHot, ...freq20Hot]);
    ...
    // ── 近況觀察手 ──
    // 專長：追最近1-3期出現的號碼，抓最即時的動態
    // 底層思維：賓果節奏快，最近1-3期的號碼最有參考價值
    // 連續出現次數越多加分越高（球員發燙狀態）
    const recentFreq = new Map();
    // 最新一期權重最高（×3），次期（×2），第三期（×1）
    (latest || []).slice(0, 20).forEach(n => {
      recentFreq.set(Number(n), (recentFreq.get(Number(n)) || 0) + 3);
    });
    freq20Hot.slice(0, 5).forEach(n => {
      recentFreq.set(Number(n), (recentFreq.get(Number(n)) || 0) + 1);
    });
    const recentSorted = [...recentFreq.entries()]
      .sort((a, b) => b[1] - a[1])
      .map(([n]) => n)
      .filter(n => !usedSet.has(n));
    tryAdd(recentSorted);
    if (picked.length < numCount) tryAdd([...freq20Hot.slice(0, 15)]);
...
export function buildBingoV1Strategies(allRows = [], strategyWeightMap = {}, starCount = 4, marketSnapshot = {}, recent10Stats = {}, dynamicStrategyKeys = [], forcedGroupCount = null) {
  ...
  if (starCount === 3) {
    ...
    // ✅ 步驟七v5串聯：優先使用步驟七直接計算的動態角色分配
    // 步驟五數據 → 步驟七計算角色分配 → 步驟六盤面做fallback
    const dynamicRoleFromStep7 = Array.isArray(marketSnapshot?.dynamic_role_allocation)
      ? marketSnapshot.dynamic_role_allocation
      : null;

    if (dynamicRoleFromStep7 && dynamicRoleFromStep7.length === 8) {
      dynamicRoles = dynamicRoleFromStep7;
      console.log(`[step7 v5] 直接使用動態角色分配: ${dynamicRoles.join(',')}`);
    } else {
      dynamicRoles = decideDynamicRoleAllocation(marketCondition, totalGroups, roleWeights);
      console.log(`[step6 fallback] 使用盤面判斷: ${dynamicRoles.join(',')}`);
    }

    boardState = analyzeBoardState(allRows, marketSnapshot);
    console.log(`[3star] 盤面: streak=${marketCondition.streakStrength} hotConc=${marketCondition.hotConcentration} cold=${marketCondition.coldPressure}`);
    console.log(`[3star] 最終角色: ${dynamicRoles.join(', ')}`);
  }
  ...
  if (starCount === 3 && marketCondition && dynamicRoles) {
    ...
    // ✅ v20 修復：各角色對應的合法策略清單，useKeys 不夠時從同角色策略補上
    const ROLE_FALLBACK_MAP = {
      'hot':       ['dynamic_hot_5', 'dynamic_hot_6', 'hot_zone_cover_1', 'zone_rotation_hot_2'],
      'cold':      ['dynamic_cold_5', 'dynamic_cold_6', 'rebound', 'cold_zone_cover'],
      'zone_fill': ['dynamic_zone_fill_6', 'zone_rotation_hot_2', 'mix_zone_3'],
      'recent':    ['dynamic_recent_6'],
      'gap_zone':  ['dynamic_gap_zone_5', 'mix_gap'],
      'scatter':   ['mix_gap', 'mix_zone_3'],
      'streak':    ['dynamic_hot_5', 'zone_rotation_hot_2'],
      'balance':   ['mix_zone_3', 'mix_gap'],
    };
    ...
    dynamicRoles.forEach((role, idx) => {
      ...
      // ✅ v20 修復：useKeys 不夠時從同角色的合法策略補上，不造假 key
      let strategyKey = useKeys[idx];
      if (!strategyKey) {
        const candidates = (ROLE_FALLBACK_MAP[role] || []).filter(k => useKeys.includes(k));
        strategyKey = candidates[0] || useKeys[0] || 'mix_gap';
        console.log(`[v20 fallback] idx=${idx} role=${role} → 使用 ${strategyKey} 補位`);
      }
      rawStrategies.push({
        key: strategyKey,
        label: strategyKey,
  ...
  if (starCount === 3 && deduped.length >= 3) {
    ...
    // ✅ 修復重複問題：收集前6組已用的號碼，第7、8組不重複
    const usedByFront = new Set();
    deduped.slice(0, deduped.length - 2).forEach(s => {
      (s.nums || []).forEach(n => usedByFront.add(Number(n)));
    });

    const hotNums1 = pickHotCoverNums(hotFreq, usedByFront, numCount);
    hotNums1.forEach(n => usedByFront.add(n));
    const hotNums2 = pickHotCoverNums(hotFreq, usedByFront, numCount);
    ...
    const freq20Cold = Array.isArray(marketSnapshot?.freq20_cold_nums) ? marketSnapshot.freq20_cold_nums : [];
    const gapNums = extractGapNumbers(marketSnapshot, 15);
    // ✅ 排除前7組已用號碼（包含 hot_zone_cover_1）
    const coldCandidates = [...new Set([...freq20Cold, ...gapNums])]
      .filter(n => !usedByFront.has(Number(n)));
    ...
    if (coldNums.length < 3) {
      ...
      const freqSortedCold = Object.entries(numFreqMap)
        .map(([n, f]) => ({ n: Number(n), f: Number(f) }))
        .filter(x => x.n >= 1 && x.n <= 80 && !coldNums.includes(x.n) && !usedByFront.has(x.n))
        .sort((a, b) => a.f - b.f)
        .map(x => x.n);
