...
function decideDynamicRoleAllocation(condition = {}, totalGroups = 8) {
  ...
  if (streakStrength >= 5) {
  ...
  } else if (streakStrength >= 3) {
    // 連號中等 → 一半連號一半分散
    roles = ['streak', 'hot', 'hot', 'cold', 'zone_fill', 'scatter', 'recent', 'scatter'];
    ...
    // 號碼集中在某個區段 → 多填補其他區段
    roles = ['zone_fill', 'zone_fill', 'zone_fill', 'hot', 'cold', 'streak', 'scatter', 'recent'];
...
// 根據角色選號
function buildNumsByRole(role = 'scatter', condition = {}, usedNums = new Set(), numCount = 3, seed = 0) {
  ...
  const tryAdd = (candidates) => {
    for (const n of candidates) {
      ...
      if (!usedSet.has(n) && n >= 1 && n <= 80 && !picked.includes(n)) {
        picked.push(n);
        usedSet.add(n);
      }
    }
  ...
  if (role === 'streak') {
    // 追連號：streak4 > streak3 > streak2 > hot5
    tryAdd([...streak4, ...streak3, ...streak2, ...hot5]);
  } else if (role === 'hot') {
    // 追熱號：hot5 > hot10 > hot20
    tryAdd([...hot5, ...hot10, ...hot20]);
  } else if (role === 'cold') {
    // 追冷號回補：gap號碼（久未出現）
    tryAdd([...gapNums]);
    // 不夠就從非熱號補
    const hot10Set = new Set(hot10);
    const coldPool = Array.from({length: 80}, (_, i) => i + 1)
      .filter(n => !hot10Set.has(n) && !usedSet.has(n));
    tryAdd(coldPool.sort(() => Math.random() - 0.5));
  } else if (role === 'zone_fill') {
    // 填補冷門區段：找出最近3期最少出現的區段，從那個區段選號
    ...
    const zonePool = Array.from({length: hi - lo + 1}, (_, i) => lo + i)
      .filter(n => !usedSet.has(n));
    // 從這個冷門區段選，再從其他區段補足
    tryAdd(zonePool.sort(() => Math.random() - 0.5));
    tryAdd(Array.from({length: 80}, (_, i) => i + 1).sort(() => Math.random() - 0.5));
  } else if (role === 'recent') {
    // 追最近開獎號碼中的熱號
    tryAdd([...latest, ...hot5]);
  } else {
    // scatter：完全分散，從各區段各選一個
    const zones = [[1,20],[21,40],[41,60],[61,80]];
    const shuffledZones = zones.sort(() => Math.random() - 0.5);
    for (const [lo, hi] of shuffledZones) {
      if (picked.length >= numCount) break;
  ...
  if (picked.length < numCount) {
    const allPool = Array.from({length: 80}, (_, i) => i + 1)
      .filter(n => !usedSet.has(n))
      .sort(() => Math.random() - 0.5);
    tryAdd(allPool);
  }
...
export function buildBingoV1Strategies(allRows = [], strategyWeightMap = {}, starCount = 4, marketSnapshot = {}, recent10Stats = {}, dynamicStrategyKeys = [], forcedGroupCount = null) {
  ...
  function pickHotCoverNums(hotFreq, globalUsedNums, numCount = 3) {
    const sorted = Object.entries(hotFreq)
      .sort((a, b) => b[1] - a[1])
      .map(([n]) => Number(n))
  ...
  if (starCount === 3 && marketCondition && dynamicRoles) {
    ...
    dynamicRoles.forEach((role, idx) => {
      const nums = buildNumsByRole(role, marketCondition, globalUsedForRoles, numCount, idx * 17);
