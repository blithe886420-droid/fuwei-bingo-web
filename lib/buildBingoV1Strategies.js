 ...
 * 根據 SQL 回測結論調整：
 *
 * 1. 爆發期連續>=2期 → 強制切換醞釀期邏輯（命中率從第3期開始掉到0%）
 * 2. 選號以第3名熱號為核心（SQL21：第3名貢獻度最高70.6%）
 * 3. 四週期→三週期→兩週期自動降級選號
 *
 * 狀態判斷：
 * 🔥 爆發期：四週期號碼>=4顆（連續<2期）
 * ⚡ 醞釀期：三週期號碼>=3顆，或爆發期連續>=2期強制切換
 * 👀 觀察期：兩週期號碼>=3顆
 * ⏸️ 冷場：以上都不符合
 */
...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  const prevPool = (recentPredictions[0]?.hot_pool || '').split(',').filter(Boolean);
