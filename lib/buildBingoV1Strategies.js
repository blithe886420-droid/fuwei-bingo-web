...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  if (forcedSwitch && threeNums.length >= 3) {
    ...
    // ★ 非高命中時段選號策略（SQL驗證）
    // 61-80區間命中率12%、1-20區間命中率11%，都高於其他區間
    // 優先找61-80，其次找1-20，都沒有才用正常排序
    ...
    if (zone61to80.length >= 3) {
      // 61-80有3顆以上：優先用61-80，不足補其他
      const combined = [...zone61to80, ...zone1to20, ...otherZone].slice(0, 5).map(s => s.n).sort((a, b) => a - b);
      top5 = combined;
      console.log(`[buildBingoGroups] 非高命中時段 優先61-80 top5=${top5.join(',')}`);
    } else if (zone1to20.length >= 3) {
      // 1-20有3顆以上：用1-20，不足補其他
      const combined = [...zone1to20, ...zone61to80, ...otherZone].slice(0, 5).map(s => s.n).sort((a, b) => a - b);
      top5 = combined;
      console.log(`[buildBingoGroups] 非高命中時段 優先1-20 top5=${top5.join(',')}`);
    } else {
      // 兩個區間都不夠：正常取w1次數最多的前5顆
