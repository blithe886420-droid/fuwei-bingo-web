...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // ★ V0613-7：候選池前10名 + G策略8組。
  // 富緯提出"打開戰略布局"要求，窮舉候選池前10名C(10,3)=120種位置組合，
  // 用425+期資料找出avg_pnl_if_solo最好的8種：
  // (1,5,8)(1,5,10)(4,5,8)(1,4,5)(4,5,7)(3,5,10)(2,4,7)(2,4,10)
  // 429期：avg_pnl從V0613-6的-75.88改善為-45.70(大幅改善)。
  // 用3000期大樣本複驗：V0613-6(前7名8組)=-90.52，本策略=-84.22，
  // 改善幅度縮小為約6.3元/期，但方向穩定(兩個樣本量下都優於V0613-6)，
  // 判定為今天驗證範圍內最值得採用的方案。
  // 同樣注意：hotPool10Ranked必須維持「熱度排序」，不可依號碼大小重排，
  // 否則「位置1-10」對應關係會跟驗證時不一致。
  const hotPool10Ranked = allCandidates.slice(0, 10).map(s => s.n); // 依w1cnt/period排序，rank1~10
  const hotPool7 = [...hotPool10Ranked.slice(0, 7)].sort((a, b) => a - b); // 供hot_pool欄位顯示(前7名，號碼排序)
  ...
  const finalCombos = hotPool10Ranked.length === 10
    ? bestPositionCombos.map(([i, j, k]) =>
        [hotPool10Ranked[i - 1], hotPool10Ranked[j - 1], hotPool10Ranked[k - 1]].sort((a, b) => a - b)
      )
    : []; // 候選池不足10顆時，安全防呆(回傳空陣列，記為skipped)

  console.log(`[buildBingoGroups] spiderMode=${spiderMode} top5=${top5.join(',')} hotPool10Ranked=${hotPool10Ranked.join(',')} 真信號=${trueSignalCount} 組數=${finalCombos.length}`);
