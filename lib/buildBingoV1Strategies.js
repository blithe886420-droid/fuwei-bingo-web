...
function decideDynamicRoleAllocation(condition = {}, totalGroups = 8) {
  ...
  if (marketPhase === 'continuation') {
    // ✅ 根據連號強度動態調整：streak=0時不派streak角色
    if (streakStrength >= 5) {
      // 有連號：streak為主
      roles = ['streak', 'hot', 'streak', 'cold', 'hot', 'recent', 'zone_fill', 'scatter'];
    } else if (hotConcentration >= 30) {
      // 熱區高度集中：hot+cold為主（對應今天 hotConc=58 的盤面）
      roles = ['hot', 'cold', 'hot', 'cold', 'recent', 'zone_fill', 'scatter', 'hot'];
    } else {
      // 一般輪動：均衡配置
      roles = ['hot', 'cold', 'recent', 'hot', 'zone_fill', 'cold', 'scatter', 'recent'];
    }
