/**
 * buildBingoV1Strategies.js - V0614-8
 *
 * 蜘蛛感知系統 + 六層觸發條件選號策略
 *
...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  const triggerLossFast = prevFullLoss && isFastBurst;   // 新：槓龜後+換手5
  // 高號7+(單期)複合：和值900-949更強(+25.83)，和值<900也有效(+4.84)
  ...
  // ── 8組位置組合 ──
  const combos_burstBrew = [   // 換手5+醞釀：+186.21
    [3,4,8],[4,7,8],[1,3,10],[6,9,10],[3,4,7],[2,4,8],[1,6,10],[1,7,10],
  ];
  const combos_slowRich = [    // 換手1+TQ22+：+23.81
    [3,5,7],[1,3,5],[3,5,10],[1,3,10],[1,5,10],[2,6,8],[4,5,8],[1,4,5],
  ];
  const combos_spider = [      // 蜘蛛感知(前12名)：+390
    [4,11,12],[4,9,11],[1,5,10],[6,7,9],[1,5,8],[1,6,10],[4,5,12],[7,9,11],
  ];
  const combos_tailSlow = [    // 同尾5+後+換手1：+96.88
    [2,7,9],[2,3,7],[2,8,9],[2,6,9],[6,8,9],[1,5,8],[3,5,10],[4,5,7],
  ];
  const combos_lossFast = [    // 槓龜後+換手5：+53.13(新，同combos_burstBrew方向類似)
    [3,4,8],[4,7,8],[1,3,10],[6,9,10],[3,4,7],[2,4,8],[1,6,10],[1,7,10],
  ];
  const combos_highZoneRich = [ // 高號7+單期+和值900-949：+25.83(新複合)
    [5,6,8],[1,6,8],[1,5,6],[3,5,8],[6,8,10],[1,3,5],[6,7,8],[7,8,10],
  ];
  const combos_highZone = [    // 高號7+單期(和值<900)：+4.84
    [8,9,10],[1,3,10],[5,9,10],[5,8,9],[1,5,6],[1,6,8],[2,8,10],[7,8,9],
  ];
  const combos_oddOdd = [      // 奇奇X：-25(相對最好，166筆)
    [6,7,9],[1,6,7],[1,7,9],[1,2,7],[1,5,8],[4,6,9],[2,4,6],[2,4,8],
  ];
  const combos_standard = [    // 標準G策略：-84
  ...
  const activeMode = triggerBurstBrew ? 'burst_brew'
    ...
    : 'standard';
  console.log(`[buildBingoGroups] mode=${activeMode} changed=${changedNums} TQ=${totalQualified} pos=${position} prevHighZone=${prevHighZoneCount} prevSum=${prevSumVal} prevMaxTail=${prevMaxTail} 組數=${finalCombos.length}`);
  ...
  return finalCombos.map(combo => {
    ...
    return {
      ...
      meta: {
        ...
        spider_sense_active: spiderSenseActive || triggerBurstBrew || triggerSlowRich || triggerHighZone || triggerHighZoneRich || triggerTailSlow || triggerLossFast || triggerOddOdd,
