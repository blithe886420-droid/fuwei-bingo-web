...
function analyzeBoardState(allRows = [], marketSnapshot = {}) {
  ...
  // ✅ fix：區間均衡保護，每區間最多6顆（24顆/4區間），避免高號爆滿
  // 原本每區間8顆導致24顆裡高號可佔17顆，中3很難
  for (const { n, score } of ranked) {
    ...
    if (zoneCount >= 6) continue;  // 原本8改為6，強制四個區間均衡
    selected.push(n);
  ...
  // ✅ fix：確保 24 顆裡低號(1-40)至少 8 顆、高號(41-80)至少 8 顆
  // 8 組每組 3 顆，至少需要 8 顆低號才能保證每組都有低號
  const lowNums = selected.filter(n => n <= 40);
  const highNums = selected.filter(n => n > 40);

  if (lowNums.length < 8) {
    // 低號不足，從 ranked 裡補充低號
    const needed = 8 - lowNums.length;
    let added = 0;
    for (const { n } of ranked) {
      ...
      if (n <= 40 && !selected.includes(n)) {
        ...
        const worstHighIdx = selected.map((x, i) => ({ x, i, s: scores[x] || 0 }))
          ...
          .sort((a, b) => a.s - b.s)[0];
        if (worstHighIdx) {
          selected.splice(worstHighIdx.i, 1, n);
          added++;
        }
      }
    }
  ...
  if (highNums.length < 8) {
    // 高號不足，從 ranked 裡補充高號
    const needed = 8 - highNums.length;
    let added = 0;
    for (const { n } of ranked) {
      if (added >= needed) break;
      if (n > 40 && !selected.includes(n)) {
        const worstLowIdx = selected.map((x, i) => ({ x, i, s: scores[x] || 0 }))
          ...
          .sort((a, b) => a.s - b.s)[0];
        if (worstLowIdx) {
          selected.splice(worstLowIdx.i, 1, n);
          added++;
        }
      }
    }
