...
// ✅ 通用選號函式：給 strategy_pool 裡沒有對應固定策略的 key 使用
function buildHotChaseForKey(strategyKey = '', todayRows, recent20, historyRows, optimizerWeight, marketProfile, marketSnapshot = {}) {
  const hot5 = extractHotNumbers(marketSnapshot, 'hot_5', 5);
  const hot10 = extractHotNumbers(marketSnapshot, 'hot_10', 10);
  const streak3 = extractStreakNumbers(marketSnapshot, 'streak3');
  const streak2 = extractStreakNumbers(marketSnapshot, 'streak2');
  const gapNums = extractGapNumbers(marketSnapshot, 10);
  ...
  // 根據策略 key 的 token 決定選號偏向
  if (tokens.includes('hot') || tokens.includes('repeat') || tokens.includes('chase')) {
    hot5.forEach(n => { const k = String(n).padStart(2,"0"); merged[k] = (merged[k] || 0) + 12; });
    hot10.forEach(n => { const k = String(n).padStart(2,"0"); merged[k] = (merged[k] || 0) + 6; });
  } else if (tokens.includes('gap') || tokens.includes('cold') || tokens.includes('rebound') || tokens.includes('reverse')) {
    gapNums.forEach(n => { const k = String(n).padStart(2,"0"); merged[k] = (merged[k] || 0) + 10; });
  } else if (tokens.includes('streak') || tokens.includes('pattern') || tokens.includes('structure')) {
    streak3.concat(streak2).forEach(n => { const k = String(n).padStart(2,"0"); merged[k] = (merged[k] || 0) + 12; });
  } else if (tokens.includes('zone') || tokens.includes('balanced') || tokens.includes('cluster')) {
    hot10.forEach(n => { const k = String(n).padStart(2,"0"); merged[k] = (merged[k] || 0) + 8; });
  } else {
    hot5.forEach(n => { const k = String(n).padStart(2,"0"); merged[k] = (merged[k] || 0) + 8; });
  }

  const sorted = sortByScore(merged).map(x => ({ ...x, zone: getZone(x.num), tail: getTail(x.num) }));
  const nums = buildStructuredGroup(sorted, marketProfile, strategyKey, { limit: 20 }, marketSnapshot);

  return {
    key: strategyKey,
    label: strategyKey.split('_').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(' '),
    nums,
    reason: `動態策略: ${strategyKey}`,
    meta: { model: 'v3_dynamic', optimizerWeight, focus: 'dynamic', marketPhase: marketSnapshot?.market_phase || 'rotation' }
  };
}

// ✅ 根據 hit3_rate + 覆蓋率回饋動態決定每個策略出幾組
function decideGroupCountByPerformance(recent10Stats = {}, marketSnapshot = {}, useKeys = []) {
...
export function buildBingoV1Strategies(allRows = [], strategyWeightMap = {}, starCount = 4, marketSnapshot = {}, recent10Stats = {}, dynamicStrategyKeys = []) {
  ...
  const { allocation, totalGroups } = decideGroupCountByPerformance(recent10Stats, marketSnapshot, useKeys);

  // 依照 allocation 展開策略組合
  const rawStrategies = [];
  useKeys.forEach(key => {
    // 有固定策略直接用，沒有的用通用熱號選號
    const s = baseStrategyMap.get(key) || buildHotChaseForKey(key, todayRows, recent20, historyRows, weights.hot_chase, marketProfile, marketSnapshot);
