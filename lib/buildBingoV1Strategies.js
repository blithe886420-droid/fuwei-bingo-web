/**
 * buildBingoV1Strategies.js - V0614-6
 *
 * 蜘蛛感知系統 + 六層觸發條件選號策略
 *
...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  const triggerHighZone = prevHighZoneCount >= 7 && prev2HighZoneCount < 7; // 精確：單期高號7+
  ...
  const activeMode = triggerBurstBrew ? 'burst_brew'
    ...
    : 'standard';
  console.log(`[buildBingoGroups] mode=${activeMode} changed=${changedNums} TQ=${totalQualified} pos=${position} prevHighZone=${prevHighZoneCount} prevMaxTail=${prevMaxTail} 組數=${finalCombos.length}`);
  ...
  return finalCombos.map(combo => {
    ...
    return {
      ...
      meta: {
        ...
        spider_sense_active: spiderSenseActive || triggerBurstBrew || triggerSlowRich || triggerHighZone || triggerTailSlow,
