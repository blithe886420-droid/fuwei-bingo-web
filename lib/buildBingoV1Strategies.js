...
// hot_windows.hot_5.map 是 {"1":3,"2":1,...} 格式，取分數最高的 N 個
function extractHotNumbers(marketSnapshot, windowKey = 'hot_5', topN = 5) {
  ...
  // 支援兩種格式：array 或 {map: {...}} 或 {numbers: [...]}
  if (Array.isArray(windowData)) return windowData.slice(0, topN);
  if (Array.isArray(windowData?.numbers)) return windowData.numbers.slice(0, topN);

  const mapData = windowData?.map || windowData;
...
function extractStreakNumbers(marketSnapshot, streakKey = 'streak3') {
  const val = marketSnapshot?.streaks?.[streakKey] || marketSnapshot?.[streakKey];
  if (Array.isArray(val)) return val;
  if (typeof val === 'object' && val !== null) {
    // 可能是 {num: streak_count} 格式
    return Object.keys(val).map(Number).filter(Number.isFinite);
  }
  return [];
}

// ✅ 從 marketSnapshot 解析 gap/cold 號碼
function extractGapNumbers(marketSnapshot, topN = 15) {
  const gap = marketSnapshot?.gap_numbers;
  if (Array.isArray(gap)) return gap.slice(0, topN);
  return [];
}

function extractColdNumbers(marketSnapshot, topN = 10) {
  const cold = marketSnapshot?.cold_numbers;
  if (Array.isArray(cold)) return cold.slice(0, topN);
  return [];
}
