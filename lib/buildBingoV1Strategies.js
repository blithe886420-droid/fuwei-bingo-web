/**
 * buildBingoV1Strategies.js - V0612-1
 *
 * 蜘蛛感知系統 + 太極四象框架
 *
 * V0611-3 根據SQL個案分析優化：
 * 1. 移除爆發期強制切換（第3、4期命中率25%，比第1、2期更高）
 * 2. 高命中時段+換手快 = 不出手（命中率0%，假信號）
 * 3. 醞釀期第5期降低信心（波浪型，第5期命中率0%）
 * 4. 號碼集中才是真獵物（分散時命中率只有4-12%）
 * 5. hot_pool改為取7顆（中2差點中3：差的號碼100%在pool外，覆蓋範圍太窄）
 */
...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  } else if (isBadBoard || position === '觀察期' || isBrewLowPoint) {
    // 盤面不對勁、觀察期、醞釀第5期低谷：取第3-7名起熱號
    const rank3to7 = allCandidates.slice(2, 7).map(s => s.n).sort((a, b) => a - b);
    top5 = rank3to7.length >= 3 ? rank3to7 : allCandidates.slice(0, 5).map(s => s.n).sort((a, b) => a - b);
    spiderMode = isBrewLowPoint ? 'brew_low_point' : 'bad_board';
  ...
  // ★ 用top7產生更多組合，取前8組
  // 讓hot_pool更寬，覆蓋更多可能的中3號碼
  const hotPool7 = allCandidates.slice(0, 7).map(s => s.n).sort((a, b) => a - b);
  const allCombos = makeCombos(hotPool7);
  ...
  const finalCombos = [...priorityCombos, ...otherCombos].slice(0, 8);

  console.log(`[buildBingoGroups] spiderMode=${spiderMode} top5=${top5.join(',')} hotPool7=${hotPool7.join(',')} 真信號=${trueSignalCount} 組數=${finalCombos.length}`);
