...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // ★ V0613-6：固定位置組合策略。
  // 用425+期歷史資料窮舉C(7,3)=35種位置組合(熱度排名1-7的搭配)，
  // 依用25/50/500真實賺賠規則算出的hit2plus_pct/avg_pnl_if_solo排序，
  // 固定取表現最好的8種位置組合：
  // (1,4,5)(2,4,6)(2,4,7)(3,4,5)(4,5,6)(4,5,7)(1,3,5)(4,6,7)
  // 427期回測：avg_pnl從現行的-96.13元/期改善為-75.88元/期，
  // 打平或以上比例從13.38%提升至15.93%。
  // 注意：hotPool7Ranked必須維持「熱度排序」(不可依號碼大小重排)，
  // 否則「位置1-7」的對應關係會跟驗證時不一致。
  const hotPool7Ranked = allCandidates.slice(0, 7).map(s => s.n); // 依w1cnt/period排序，rank1~7
  const hotPool7 = [...hotPool7Ranked].sort((a, b) => a - b); // 供hot_pool欄位顯示(號碼排序)

  const bestPositionCombos = [
    [1, 4, 5], [2, 4, 6], [2, 4, 7], [3, 4, 5],
    [4, 5, 6], [4, 5, 7], [1, 3, 5], [4, 6, 7],
  ];

  const finalCombos = hotPool7Ranked.length === 7
    ? bestPositionCombos.map(([i, j, k]) =>
        [hotPool7Ranked[i - 1], hotPool7Ranked[j - 1], hotPool7Ranked[k - 1]].sort((a, b) => a - b)
      )
    : []; // 候選池不足7顆時(理論上不會發生，因top5.length<3已提前return)，安全防呆

  console.log(`[buildBingoGroups] spiderMode=${spiderMode} top5=${top5.join(',')} hotPool7Ranked=${hotPool7Ranked.join(',')} 真信號=${trueSignalCount} 組數=${finalCombos.length}`);
