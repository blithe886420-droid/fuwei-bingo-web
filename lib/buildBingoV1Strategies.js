...
// ✅ v17 第四階段：盤面感知選號（戰術層）
// 不再問「這個策略應該選什麼號碼」
// 改為問「這80顆號碼裡，哪24顆今期最值得覆蓋」
// 先對80顆號碼各自打分，再把最高分的24顆分配給8組
// ============================================================
function analyzeBoardState(allRows = [], marketSnapshot = {}) {
  ...
  for (let n = 1; n <= 80; n++) {
    ...
    // 2. 熱號加分
    if (hot5.includes(n)) score += 8.0;
    else if (hot10.includes(n)) score += 4.0;
    else if (hot20.includes(n)) score += 2.0;

    // 3. 連號加分（散亂期降低，集中期保留）
    if (streak4.includes(n)) score += marketPhase === 'continuation' ? 6.0 : 2.0;
    else if (streak3.includes(n)) score += marketPhase === 'continuation' ? 4.0 : 1.5;

    // 4. 回補分（散亂期加重，集中期降低）
    const gap = gapAnalysis[n];
    if (gap && gap.overdueScore > 0) {
      const overdueWeight = marketPhase === 'continuation' ? 1.0 :
                            marketPhase === 'chaos' ? 4.0 : 3.0;
      score += gap.overdueScore * overdueWeight;
    }

    // 5. 冷號加分（散亂期特別有效）
    const coldIdx = freq20Cold.indexOf(n);
    if (coldIdx >= 0) {
      const coldWeight = marketPhase === 'continuation' ? 0.5 : 2.0;
      score += (10 - Math.min(coldIdx, 9)) * coldWeight * 0.3;
    }

    // 6. 近20期高頻加分
    const hotIdx = freq20Hot.indexOf(n);
    if (hotIdx >= 0) {
      score += (20 - Math.min(hotIdx, 19)) * 0.3;
    }

    // 7. 尾數多樣性：各尾數都能入選（防止號碼集中在某個尾數）
    const tail = n % 10;
    // 不在這裡加，在分配時考慮
  ...
  const selected = [];
  const selectedTails = new Map(); // tail -> count
  const selectedZones = new Map(); // zone(1-4) -> count

  // 第一輪：按分數選，但限制同尾數最多3顆、同區間最多8顆
  for (const { n, score } of ranked) {
    ...
    if (tailCount >= 3) continue; // 同尾數最多3顆
    if (zoneCount >= 8) continue; // 同區間最多8顆
    selected.push(n);
...
function decideDynamicRoleAllocation(condition = {}, totalGroups = 8) {
  const {
  ...
  // ✅ v16 方向四：跨組協調機制
  // 根據近期開獎分布，動態決定冷/熱/zone_fill 的比例
  // 讓8組合起來的號碼分布模式跟近期開獎趨勢吻合
  const totalRecentNums = recentZoneDist ? recentZoneDist.reduce((a, b) => a + b, 0) : 0;
  const zoneBiasStrong = totalRecentNums > 0 && Math.max(...(recentZoneDist || [])) / totalRecentNums > 0.35;
  const coldPressureHigh = coldPressure >= 12;

  // ✅ 優先用 market_phase 決定大方向，再用細部指標微調
  if (marketPhase === 'continuation') {
    if (streakStrength >= 5) {
      roles = ['streak', 'streak', 'hot', 'hot', 'recent', 'hot', 'cold', 'scatter'];
    } else {
      roles = ['hot', 'hot', 'hot', 'streak', 'recent', 'cold', 'zone_fill', 'scatter'];
    }

  } else if (marketPhase === 'bias') {
    if (zoneBias >= 25) {
      roles = ['zone_fill', 'zone_fill', 'hot', 'hot', 'cold', 'streak', 'recent', 'scatter'];
    } else {
      roles = ['hot', 'zone_fill', 'zone_fill', 'cold', 'streak', 'recent', 'hot', 'scatter'];
    }

  } else if (marketPhase === 'chaos') {
    roles = ['scatter', 'scatter', 'scatter', 'zone_fill', 'cold', 'hot', 'recent', 'streak'];

  } else {
    // rotation（輪動盤）
    // ✅ v16：冷號壓力高時增加 cold 比例（gap_zone_rotation 表現最好的訊號）
    if (coldPressureHigh && streakStrength < 5) {
      roles = ['cold', 'cold', 'hot', 'zone_fill', 'scatter', 'recent', 'hot', 'streak'];
    } else if (streakStrength >= 5) {
      roles = ['streak', 'streak', 'hot', 'cold', 'zone_fill', 'scatter', 'recent', 'hot'];
    } else if (hotConcentration >= 4) {
      roles = ['hot', 'hot', 'streak', 'cold', 'cold', 'zone_fill', 'scatter', 'recent'];
    } else if (coldPressure >= 15) {
      roles = ['cold', 'cold', 'cold', 'hot', 'streak', 'zone_fill', 'scatter', 'recent'];
    } else if (latestHotOverlap >= 8) {
      roles = ['hot', 'hot', 'hot', 'streak', 'cold', 'zone_fill', 'scatter', 'recent'];
    } else if (latestHotOverlap <= 3) {
      roles = ['scatter', 'scatter', 'zone_fill', 'zone_fill', 'cold', 'hot', 'streak', 'recent'];
    } else {
      roles = ['hot', 'cold', 'streak', 'zone_fill', 'scatter', 'recent', 'hot', 'cold'];
    }
  }

  console.log(`[role_alloc] phase=${marketPhase} streak=${streakStrength} hotConc=${hotConcentration} coldHigh=${coldPressureHigh} zoneBiasStrong=${zoneBiasStrong} → ${roles.slice(0, totalGroups).join(',')}`);
...
function buildNumsByRole(role = 'scatter', condition = {}, usedNums = new Set(), numCount = 3, seed = 0, marketSnapshot = {}) {
  ...
  if (role === 'streak') {
    // 追連號：streak4 > streak3 > streak2 > freq20Hot > hot5
    tryAdd([...streak4, ...streak3, ...streak2, ...freq20Hot, ...hot5]);

  } else if (role === 'hot') {
    // ✅ 修復：追熱號優先用近20期頻率數據，比舊的 hot5/10 更準確
    tryAdd([...freq20Hot, ...hot5, ...hot10, ...hot20]);

  } else if (role === 'cold') {
    // 追冷號回補：freq20Cold（近20期低頻）> gapNums（久未出現）
    tryAdd([...freq20Cold, ...gapNums]);
    // 不夠就從非熱號補（頻率排序，從低頻往上選）
    const freqSortedCold = [...freqSortedAll].reverse().filter(n => !usedSet.has(n));
    tryAdd(freqSortedCold);

  } else if (role === 'zone_fill') {
    // ✅ 修復：填補冷門區段，從高頻號碼裡選該區段的號碼，不再完全隨機
    const minZoneIdx = recentZoneDist.indexOf(Math.min(...recentZoneDist));
    const zoneRanges = [[1,20],[21,40],[41,60],[61,80]];
    const [lo, hi] = zoneRanges[minZoneIdx];
    // 從高頻號碼裡篩選該區段的號碼（頻率優先，不隨機）
    ...
    // 追最近開獎號碼中的熱號：freq20Hot > latest > hot5
    tryAdd([...freq20Hot.slice(0, 10), ...latest, ...hot5]);
...
export function buildBingoV1Strategies(allRows = [], strategyWeightMap = {}, starCount = 4, marketSnapshot = {}, recent10Stats = {}, dynamicStrategyKeys = [], forcedGroupCount = null) {
  ...
  if (starCount === 3) {
    ...
    dynamicRoles = decideDynamicRoleAllocation(marketCondition, totalGroups);
  ...
  if (starCount === 3 && marketCondition && dynamicRoles) {
    ...
    if (boardState && boardState.groups.length >= totalGroups) {
      // 使用盤面分析的預分配結果
      boardState.groups.slice(0, totalGroups).forEach((groupNums, idx) => {
        const nums = groupNums.map(Number).filter(n => n >= 1 && n <= 80);
        nums.forEach(n => globalUsedForRoles.add(n));
        const strategyKey = useKeys[idx] || `board_${idx + 1}`;
        const role = dynamicRoles[idx] || 'scatter';
        rawStrategies.push({
          key: strategyKey,
          label: `${role.toUpperCase()}｜${strategyKey}`,
          nums,
          reason: `盤面最優分配[${role}] top24覆蓋 盤相:${boardState.marketPhase}`,
          meta: {
            model: 'v17_board_driven',
            role,
            board_rank: idx + 1,
            market_condition: {
      ...
      // fallback：boardState 不足時，用原本的 buildNumsByRole
      dynamicRoles.forEach((role, idx) => {
        ...
        rawStrategies.push({
          ...
          label: `${role.toUpperCase()}｜${strategyKey}`,
          nums,
          reason: `盤面感知[${role}] fallback 連號:${marketCondition.streakStrength} 熱區:${marketCondition.hotConcentration} 冷號:${marketCondition.coldPressure}`,
          meta: {
            model: 'v5_market_aware',
            role,
            market_condition: {
              streakStrength: marketCondition.streakStrength,
              hotConcentration: marketCondition.hotConcentration,
  ...
  // Step 2：先對策略組去重，建立全域已用號碼集合
  const deduped = dedupeStrategies(
    rawStrategies,
    sortedCandidates,
    marketProfile,
    numCount
  ...
  // ✅ 修正 bug：在 dedupeStrategies 完成後才計算 usedNums，順序正確
  if (starCount === 3 && deduped.length >= 3) {
    const hotFreq = buildRecentHotFreq(allRows);

    // 計算前(n-2)組已用的所有號碼
    const frontGroups = deduped.slice(0, deduped.length - 2);
    const globalUsed = new Set();
    frontGroups.forEach(s => {
      (s.nums || []).forEach(n => globalUsed.add(Number(n)));
    });

    // 第1組熱區覆蓋：從前(n-2)組未用的號碼裡選
    const hotNums1 = pickHotCoverNums(hotFreq, globalUsed, numCount);
    hotNums1.forEach(n => globalUsed.add(n));

    // 第2組熱區覆蓋：從前(n-2)組 + 第1組熱區 未用的號碼裡選
    const hotNums2 = pickHotCoverNums(hotFreq, globalUsed, numCount);
    ...
    const freq20Cold = Array.isArray(marketSnapshot?.freq20_cold_nums) ? marketSnapshot.freq20_cold_nums : [];
    const gapNums = extractGapNumbers(marketSnapshot, 15);
    const coldCandidates = [...new Set([...freq20Cold, ...gapNums])].filter(n => !globalUsed.has(Number(n)));
    ...
    if (coldNums.length < 3) {
      ...
      const freqSortedCold = Object.entries(numFreqMap)
        .map(([n, f]) => ({ n: Number(n), f: Number(f) }))
        .filter(x => x.n >= 1 && x.n <= 80 && !globalUsed.has(x.n) && !coldNums.includes(x.n))
        .sort((a, b) => a.f - b.f)
        .map(x => x.n);
