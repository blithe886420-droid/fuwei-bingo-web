...
function decideDynamicRoleAllocation(condition = {}, totalGroups = 8, roleWeights = {}) {
  ...
  //   hot    = 強打者（追熱號，近期高頻）
  //   cold   = 回補手（追冷號，久未出現）
  //   streak = 連號追擊手（追連續出現的號碼）
  //   recent = 近況觀察手（追最近幾期的號碼）
  //   zone_fill = 區段填補手（填補冷門區段）
  //   scatter = 分散游擊手（廣泛覆蓋，不集中）
  //
  // 場地狀況（market_phase）：
  ...
  if (marketPhase === 'continuation') {
    // 延續盤：熱號持續中，但不能只靠熱號
    // 教練判斷：recent（近況手）最適合，因為「最近持續出現的號碼」
    // cold（回補手）也要派，因為連號盤冷號壓力在累積
    if (hasStreak) {
      // 有明顯連號：streak主攻，recent+cold輔助
      roles = ['streak', 'recent', 'cold', 'hot', 'recent', 'cold', 'zone_fill', 'scatter'];
    } else if (hotOverlapHigh) {
      // 熱號大量出現但無連號：recent+hot為主，cold保底
      roles = ['recent', 'hot', 'recent', 'cold', 'hot', 'zone_fill', 'scatter', 'cold'];
    } else {
      // 一般延續：均衡出擊
      roles = ['recent', 'cold', 'hot', 'recent', 'zone_fill', 'cold', 'scatter', 'hot'];
    }

  } else if (marketPhase === 'bias') {
    // 偏移盤：某區段特別集中，要填補冷門區段
    // 教練判斷：zone_fill是主力，cold補充，hot守住熱區
    if (zoneBiasStrong) {
      // 強烈偏移：大量zone_fill，補冷門區段
      roles = ['zone_fill', 'zone_fill', 'cold', 'hot', 'zone_fill', 'recent', 'cold', 'scatter'];
    } else {
      // 輕微偏移：均衡補位
      roles = ['zone_fill', 'cold', 'hot', 'zone_fill', 'recent', 'cold', 'hot', 'scatter'];
    }

  } else if (marketPhase === 'chaos') {
    // 混亂盤：無規律，最新一期熱號少
    // 教練判斷：scatter廣撒網，zone_fill填補，不要追熱號
    if (coldPressureHigh) {
      // 冷號壓力高：cold+scatter為主，等回補
      roles = ['cold', 'scatter', 'cold', 'zone_fill', 'scatter', 'recent', 'zone_fill', 'hot'];
    } else {
      // 一般混亂：scatter為主，廣泛覆蓋
      roles = ['scatter', 'zone_fill', 'scatter', 'cold', 'zone_fill', 'recent', 'hot', 'cold'];
    }

  } else {
    // rotation（輪動盤）：正常狀態，均衡出擊
    // 教練判斷：根據各項指標靈活調度
    if (coldPressureHigh && !hasStreak) {
      // 冷號壓力大且無連號：cold+zone_fill主攻
      roles = ['cold', 'zone_fill', 'cold', 'hot', 'zone_fill', 'recent', 'scatter', 'cold'];
    } else if (hasStreak) {
      // 有連號趨勢：streak+hot主攻
      roles = ['streak', 'hot', 'streak', 'cold', 'recent', 'zone_fill', 'scatter', 'hot'];
    } else if (hotOverlapLow) {
      // 熱號命中率低：改用scatter+zone_fill廣覆蓋
      roles = ['scatter', 'zone_fill', 'cold', 'scatter', 'zone_fill', 'recent', 'hot', 'cold'];
    } else if (hotOverlapHigh) {
      // 熱號命中率高：hot主打
      roles = ['hot', 'recent', 'hot', 'cold', 'streak', 'zone_fill', 'scatter', 'cold'];
    } else {
      // 標準輪動：各角色均衡出場
      roles = ['hot', 'cold', 'recent', 'zone_fill', 'streak', 'scatter', 'hot', 'cold'];
    }
  }
