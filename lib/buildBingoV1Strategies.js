...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // 各層級號碼數量
  const fourCount = numStats.filter(s => s.period === 4).length;
  const threeCount = numStats.filter(s => s.period === 3).length;
  const twoCount = numStats.filter(s => s.period === 2).length;

  // 判斷本期狀態（用數量判斷，不是只要有一顆就算）
  let position = '冷場期';
  let action = '跳過';
  ...
  console.log(`[buildBingoGroups] 四週期=${fourCount} 三週期=${threeCount} 兩週期=${twoCount} → ${position}`);
