/**
 * buildBingoV1Strategies.js - V0614-12
 *
 * 蜘蛛感知系統 + 六層觸發條件選號策略
 *
...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // ★ V0614-12：訊號計數器架構(太極變招系統)
  // 計算本期同時觸發幾個訊號，訊號越多→共鳴越強→用越激進的8組
  // 歷史驗證：3個訊號=-31；4+個訊號=+115；5個訊號=+270
  const s1 = isFastBurst && position === '醞釀期' ? 1 : 0;
  const s2 = isSlowTurnover1 && totalQualified >= 22 ? 1 : 0;
  const s3 = totalQualified >= 22 && changedNums <= 1 && prevIsSlowTurnover ? 1 : 0;
  const s4 = prevMaxTail >= 5 && isSlowTurnover1 ? 1 : 0;
  const s5 = prevFullLoss && isFastBurst ? 1 : 0;
  ...
  const s8 = prevConsecCount >= 8 ? 1 : 0;
  const s9 = prevIsBalanced && prev2IsBalanced ? 1 : 0;
  const s10 = prevSumVal % 2 === 1 && prev2SumVal % 2 === 1 && prev3SumVal % 2 !== 1 ? 1 : 0;
  const s11 = prevHighZoneCount >= 7 && prev2HighZoneCount < 7 ? 1 : 0;
  const totalSignals = s1+s2+s3+s4+s5+s6+s7+s8+s9+s10+s11;
  ...
  // 訊號=0時不出手
  const finalCombos = totalSignals === 0 ? []
  ...
  const activeMode = totalSignals >= 4 ? 'ultra'
    : totalSignals === 3 ? 'strong'
    : spiderSenseActive ? 'spider'
    : totalSignals > 0 ? 'standard'
    : 'skip';
  console.log(`[buildBingoGroups] mode=${activeMode} signals=${totalSignals}(${s1}${s2}${s3}${s4}${s5}${s6}${s7}${s8}${s9}${s10}${s11}) TQ=${totalQualified} pos=${position} 組數=${finalCombos.length}`);
