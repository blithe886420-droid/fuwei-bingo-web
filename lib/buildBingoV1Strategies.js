...
function buildStreakChase(todayRows, recent20, historyRows, optimizerWeight, marketProfile, marketSnapshot = {}) {
  // 優先使用 marketSnapshot 裡的 streak3/streak4 號碼
  ...
  // 建立候選池：streak4 > streak3 > attackCore > hot5 > streak2
  const priorityPool = uniq([
    ...streak4,
    ...streak3,
    ...attackCore.slice(0, 8),
    ...hot5,
    ...streak2
  ...
  return {
    ...
    label: "連熱追擊型",
    nums,
    reason: `連續熱號追擊，streak3:${streak3.length}個 streak4:${streak4.length}個`,
    meta: {
      model: "v3",
      optimizerWeight,
      focus: "streak+attack_core",
...
function decideGroupCountByPerformance(recent10Stats = {}, marketSnapshot = {}) {
  ...
  if (streak4Count >= 2 || (streak3Count >= 3 && marketPhase === 'continuation')) {
    ...
    totalGroups = 4; // 輪動盤，標準組數
  }

  // 根據近10期表現分配組數
  const strategies = [
    { key: 'streak_chase',     roi10: recent10Stats.streak_chase     ?? -0.5, baseSlots: 1 },
    { key: 'hot_chase',        roi10: recent10Stats.hot_chase         ?? -0.5, baseSlots: 1 },
    { key: 'pattern_structure',roi10: recent10Stats.pattern_structure ?? -0.5, baseSlots: 1 },
    { key: 'rebound',          roi10: recent10Stats.rebound           ?? -0.5, baseSlots: 1 },
    { key: 'zone_balanced',    roi10: recent10Stats.zone_balanced     ?? -0.5, baseSlots: 1 }
  ].sort((a, b) => b.roi10 - a.roi10); // ROI 高的排前面

  // 先每個策略分配1組，剩餘組數分給 ROI 最好的策略
  const remaining = totalGroups - strategies.length;
  const allocation = {};
  strategies.forEach(s => { allocation[s.key] = 1; });

  // 把剩餘組數依序給 ROI 最高的策略（最多加2組）
  let extra = remaining;
...
export function buildBingoV1Strategies(allRows = [], strategyWeightMap = {}, starCount = 4, marketSnapshot = {}, recent10Stats = {}) {
  ...
  // ✅ 五種策略，全部接收 marketSnapshot
  const s1 = buildStreakChase(todayRows, recent20, historyRows, weights.hot_chase, marketProfile, marketSnapshot);
  const s2 = buildHotChase(todayRows, recent20, historyRows, weights.hot_chase, marketProfile);
  const s3 = buildPatternStructure(todayRows, recent20, historyRows, weights.pattern_structure, marketProfile);
  const s4 = buildRebound(todayRows, recent20, historyRows, weights.rebound, marketProfile);
  const s5 = buildZoneBalanced(todayRows, recent20, historyRows, weights.zone_balanced, marketProfile);
