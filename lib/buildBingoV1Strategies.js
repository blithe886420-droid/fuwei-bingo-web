...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  if (forcedSwitch && threeNums.length >= 3) {
    ...
    if (isBadBoard && risingNums.length >= 5) {
      // ★ 盤面不對勁（前5顆擠在同一區間）：改用後段起熱號
      // 取起熱號裡後5顆（w1=2-3次，正在醞釀的）
      const risingTop = risingNums.slice(-5).map(s => s.n).sort((a, b) => a - b);
      top5 = risingTop;
      console.log(`[buildBingoGroups] 盤面不對勁 改用起熱號 top5=${top5.join(',')}`);

    } else if (position === '觀察期' && risingNums.length >= 5) {
      // ★ 觀察期（兩週期）：用起熱號後五顆，正在醞釀的潛力號
      const risingTop = risingNums.slice(-5).map(s => s.n).sort((a, b) => a - b);
      top5 = risingTop;
      console.log(`[buildBingoGroups] 觀察期 用起熱號後五顆 top5=${top5.join(',')}`);

    } else if (zone61to80.length >= 2) {
      // 61-80有2顆以上：取61-80最強2顆 + 其他區間最強3顆（避免號碼太集中）
      const best61to80 = zone61to80.slice(0, 2).map(s => s.n);
      const bestOther = [...zone1to20, ...otherZone].slice(0, 3).map(s => s.n);
      const combined = [...best61to80, ...bestOther].sort((a, b) => a - b);
      top5 = combined.length >= 3 ? combined : allCandidates.slice(0, 5).map(s => s.n).sort((a, b) => a - b);
      console.log(`[buildBingoGroups] 非高命中時段 61-80混合 top5=${top5.join(',')}`);

    } else if (zone1to20.length >= 2) {
      // 1-20有2顆以上：取1-20最強2顆 + 其他區間最強3顆
      const best1to20 = zone1to20.slice(0, 2).map(s => s.n);
      const bestOther = [...zone61to80, ...otherZone].slice(0, 3).map(s => s.n);
      const combined = [...best1to20, ...bestOther].sort((a, b) => a - b);
      top5 = combined.length >= 3 ? combined : allCandidates.slice(0, 5).map(s => s.n).sort((a, b) => a - b);
      console.log(`[buildBingoGroups] 非高命中時段 1-20混合 top5=${top5.join(',')}`);

    } else {
      // 正常取w1次數最多的前5顆
      top5 = allCandidates.slice(0, 5).map(s => s.n).sort((a, b) => a - b);
      console.log(`[buildBingoGroups] 非高命中時段 正常選號 top5=${top5.join(',')}`);
    }
  }
