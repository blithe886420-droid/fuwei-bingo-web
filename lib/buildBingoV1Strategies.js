/**
 * buildBingoV1Strategies.js - V0614-7
 *
 * 蜘蛛感知系統 + 六層觸發條件選號策略
 *
...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // ★ V0614-6：六層觸發條件選號策略
  // 優先級1：換手5顆+醞釀期 → +186.21(29筆)
  // 優先級2：換手1顆+TQ22+ → +23.81(63筆)
  // 優先級3：TQ22++連穩2期(蜘蛛感知) → +390(10筆)
  // 優先級4：同尾5+後+換手1顆 → +96.88(16筆)
  // 優先級5：單期高號7+(上上期非7+，精確版) → +4.84(186筆)
  // 預設：標準G策略
  // 注意：changedNums/isFastTurnover/isSlowTurnover 沿用上方第125行的宣告
  ...
  const triggerHighZone = prevHighZoneCount >= 7 && prev2HighZoneCount < 7;
  const triggerHighSum = prevSumVal >= 900;  // 新：上期和值900+ → 93筆avg_pnl=-2.42
  ...
  const combos_highSum = [  // 上期和值900+：排名5/6/8為核心
  ...
  const activeMode = triggerBurstBrew ? 'burst_brew'
    ...
    : triggerHighSum ? 'high_sum'
    : 'standard';
  console.log(`[buildBingoGroups] mode=${activeMode} changed=${changedNums} TQ=${totalQualified} pos=${position} prevHighZone=${prevHighZoneCount} prevMaxTail=${prevMaxTail} prevSum=${prevSumVal} 組數=${finalCombos.length}`);
  ...
  return finalCombos.map(combo => {
    ...
    return {
      ...
      meta: {
        ...
        spider_sense_active: spiderSenseActive || triggerBurstBrew || triggerSlowRich || triggerHighZone || triggerTailSlow || triggerHighSum,
