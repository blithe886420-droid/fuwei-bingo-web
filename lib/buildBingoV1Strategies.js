...
function decideDynamicRoleAllocation(condition = {}, totalGroups = 8) {
  ...
  if (marketPhase === 'continuation') {
    // ✅ 平衡配置：hot+streak 為主，搭配 cold 和 recent 分散
    if (streakStrength >= 5) {
      roles = ['streak', 'hot', 'streak', 'cold', 'hot', 'recent', 'zone_fill', 'scatter'];
    } else {
      roles = ['hot', 'streak', 'cold', 'hot', 'recent', 'zone_fill', 'cold', 'scatter'];
    }

  } else if (marketPhase === 'bias') {
    // ✅ 平衡配置：zone_fill+cold 為主，搭配 hot 和 recent
    if (zoneBias >= 25) {
      roles = ['zone_fill', 'cold', 'zone_fill', 'hot', 'cold', 'recent', 'streak', 'scatter'];
    } else {
      roles = ['hot', 'zone_fill', 'cold', 'zone_fill', 'recent', 'streak', 'cold', 'scatter'];
    }
