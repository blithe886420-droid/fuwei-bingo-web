/**
 * buildBingoV1Strategies.js - v31 四象太極狀態機版
 *
 * 策略：
 * - 強力出號：前一期預熱期 + 本期爆發期（pool>=5）→ 40%命中率
 * - 出號：爆發期且池上升 / 觀察期轉爆發期 / 反轉期 → 14.81%
 * - 跳過：其他 → 1.68%
 */
...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // 判斷策略
  let action = '跳過';
  if (prev1Pos === '預熱期' && currentPos === '爆發期') {
    action = '強力出號';
  } else if (
    (prev1Pos === '觀察期' && currentPos === '爆發期' && poolSize > prev1Pool) ||
    (currentPos === '爆發期' && poolSize > prev1Pool) ||
    currentPos === '反轉期'
  ) {
    action = '出號';
  }
