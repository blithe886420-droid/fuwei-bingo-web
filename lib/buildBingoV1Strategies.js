/**
 * buildBingoV1Strategies.js - V0615-2
 *
...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // ★ skip條件(最優先，直接不出手)
  const skipFastBurst = isFastBurst && position === '爆發期';  // 換手5++爆發期：-84
  const skipTQlowFast = isFastBurst && totalQualified < 25;    // TQ<25+換手5+：-121
  ...
  // 訊號=0 或 觸發skip條件 → 不出手
  const shouldSkip = totalSignals === 0 || skipFastBurst || skipTQlowFast;
  ...
  const activeMode = shouldSkip ? 'skip'
    ...
    : 'skip';
  console.log(`[buildBingoGroups] mode=${activeMode} signals=${totalSignals}(${s1}${s2}${s3}${s4}${s5}${s6}${s7}${s8}${s9}${s10}${s11}${s12}) skip=${shouldSkip} TQ=${totalQualified} pos=${position} ch=${changedNums} 組數=${finalCombos.length}`);
