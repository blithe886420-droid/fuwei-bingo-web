...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // ★ 號碼週期分析
  const numStats = [];
  const allNumStats = []; // ★ V0613-4：1-80全部號碼統計(含period=0)，供補齊24個候選池使用
  for (let n = 1; n <= 80; n++) {
    ...
    allNumStats.push({ n, w1cnt, period });
  ...
  // ★ 蜘蛛感知第一層：號碼池夠不夠
  if (totalQualified < 5) {
    console.log(`[buildBingoGroups] 號碼池不足(${totalQualified}顆)，不出手`);
    return [];
  }
  ...
  // ★ V0613-4：8組分配方式改版
  // 驗證結論(全歷史17948期)：固定7顆池C(7,3)取前8組，avg_best_hit理論值僅1.38，
  // 改為24個不重疊號碼依序切8組，avg_best_hit可達1.87(提升約35%)，
  // 且「選哪24個」對結果影響極小(熱度排序/隨機/區間平均三者結果幾乎相同)，
  // 因此維持用allCandidates(w1cnt/period排序)取前24，不足部分從
  // allNumStats(period=0號碼，依w1cnt排序)補齊，確保24個不重疊。
  const hotPool7 = allCandidates.slice(0, 7).map(s => s.n).sort((a, b) => a - b); // 保留供hot_pool欄位顯示

  const candidatesSorted = [...allCandidates].sort((a, b) => b.w1cnt - a.w1cnt || b.period - a.period);
  const candidateNumsSet = new Set(candidatesSorted.map(s => s.n));
  const fallbackSorted = allNumStats
    .filter(s => !candidateNumsSet.has(s.n))
    .sort((a, b) => b.w1cnt - a.w1cnt || a.n - b.n);

  const top24 = [...candidatesSorted, ...fallbackSorted]
    .slice(0, 24)
    .map(s => s.n);
  ...
  const finalCombos = [];
  for (let i = 0; i < 8; i++) {
    const group = top24.slice(i * 3, i * 3 + 3).sort((a, b) => a - b);
    if (group.length === 3) finalCombos.push(group);
  }

  console.log(`[buildBingoGroups] spiderMode=${spiderMode} top5=${top5.join(',')} hotPool7=${hotPool7.join(',')} top24=${top24.join(',')} 真信號=${trueSignalCount} 組數=${finalCombos.length}`);
