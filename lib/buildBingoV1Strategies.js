...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // 優先級4：上期高號區(61-80)>=7顆 → 216筆avg_pnl=-12.73
  //   8組：(4,5,10)(6,8,10)(6,7,10)(5,6,10)(1,6,8)(3,5,8)(3,4,5)(1,9,10)
  // 其他：標準G策略(前10名)
  ...
  const triggerHighZone = prevHighZoneCount >= 7;
  ...
  const combos_highZone = [
    [4,5,10],[6,8,10],[6,7,10],[5,6,10],[1,6,8],[3,5,8],[3,4,5],[1,9,10],
  ];
  ...
  const activeMode = triggerBurstBrew ? 'burst_brew'
    ...
    : 'standard';
  console.log(`[buildBingoGroups] mode=${activeMode} changed=${changedNums} TQ=${totalQualified} pos=${position} prevHighZone=${prevHighZoneCount} 組數=${finalCombos.length}`);
  ...
  return finalCombos.map(combo => {
    ...
    return {
      ...
      meta: {
        ...
        spider_sense_active: spiderSenseActive || triggerBurstBrew || triggerSlowRich || triggerHighZone,
