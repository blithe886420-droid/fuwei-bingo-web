...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // ★ V0613-5：回滾V0613-4。
  // V0613-4改版(24顆不重疊切8組)雖讓「最佳那組」avg_hit從1.37→1.76，
  // 但實際下注規則是 8組成本200元、中2得50/中3得500，
  // 需要4組以上同時中2(=200)才能打平。V0613-4「不重疊」消滅了
  // 「多組共用熱號、同時命中」的可能性：部署後63期完全沒有
  // 4組以上同時中2的期數(部署前115期有12次)，實際淨利從
  // 平均每期-40.43元惡化為-111.11元。回滾為7顆池C(7,3)取前8組，
  // 「重疊」本身才是「多組同時受益」的機制，後續優化方向應朝
  // 「提高多組同時中2機率」而非「最佳單組命中數」。
