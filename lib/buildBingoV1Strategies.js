/**
 * buildBingoV1Strategies.js - V0611-1
 *
 * SQL驗證結論：
 ...
 * 選號邏輯：
 * - 強制切換：用三週期號碼
 * - 高命中時段（9-11或16-18點）：優先選61-80區間號碼（命中率28%）
 * - 換手快+爆發期：優先選61-80區間號碼
 * - 非高命中時段：優先選61-80，其次1-20（命中率11-12%）
 * - 其他：正常取w1次數最多的前5顆
 */
...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // ★ 起熱號碼（w1=2-3次，正在醞釀，未過頂）
  // 排序：w1次數升序，取後段的起熱號（不是最熱，是正在起熱的）
  const risingNums = [...numStats]
    .filter(s => s.w1cnt >= 2 && s.w1cnt <= 3)
    .sort((a, b) => a.w1cnt - a.w1cnt || b.period - a.period);
  // 盤面不對勁的判斷：候選號碼太少（<8顆）或全部擠在同一區間
  const totalCandidates = numStats.length;
  const topZone = (() => {
  ...
  // 盤面不對勁：前5顆有4顆以上擠在同一區間（像剛才61-69的情況）
  ...
  // 12-15點命中率1.92%，策略：改用起熱號第3-7名，不用最熱的
  ...
  if (forcedSwitch && threeNums.length >= 3) {
    ...
    // ★ 高命中時段選號（SQL驗證：第4、5名貢獻33%，不能全換成61-80）
    // 策略：保留原本top5的第3、4、5名（非第一梯次），
    //       只把第1、2名換成61-80區間最強的號碼（如果有的話）
    const normalTop5 = allCandidates.slice(0, 5);
    const highZoneNums = allCandidates.filter(s => s.n >= 61 && s.n <= 80);

    if (highZoneNums.length >= 2) {
      // 有2顆以上61-80：用61-80最強2顆 + 原本第3、4、5名
      const best61to80 = highZoneNums.slice(0, 2).map(s => s.n);
    ...
    } else if (highZoneNums.length === 1) {
      // 只有1顆61-80：用61-80最強1顆 + 原本第2、3、4、5名
      const best61to80 = highZoneNums.slice(0, 1).map(s => s.n);
      const rank2345 = normalTop5.slice(1, 5).map(s => s.n);
      const combined = [...new Set([...best61to80, ...rank2345])].slice(0, 5).sort((a, b) => a - b);
      top5 = combined.length >= 3 ? combined : normalTop5.map(s => s.n).sort((a, b) => a - b);
    } else {
      // 沒有61-80：用原本top5（保留第4、5名的貢獻）
      top5 = normalTop5.map(s => s.n).sort((a, b) => a - b);
    }
    console.log(`[buildBingoGroups] 高命中時段選號 61-80有${highZoneNums.length}顆 top5=${top5.join(',')}`);

  } else if (isDeadHour) {
    // ★ 12-15點死亡時段（命中率1.92%）：切換保守策略
    // 完全跳過最熱的第1-2名，取第3-7名起熱號
    ...
    console.log(`[buildBingoGroups] 12-15點死亡時段 保守策略 第3-7名 top5=${top5.join(',')}`);
    ...
    // ★ 非高命中時段選號策略（SQL驗證 + 起熱號邏輯）
    const zone61to80 = allCandidates.filter(s => s.n >= 61 && s.n <= 80);
    const zone1to20 = allCandidates.filter(s => s.n >= 1 && s.n <= 20);
    const otherZone = allCandidates.filter(s => s.n > 20 && s.n < 61);

    // ★ 非高命中時段核心邏輯：
    // SQL驗證：第4名以後貢獻37%，第6-7名也有7.4%
    // 策略：跳過第1-2名最熱的，取第3-7名中後段起熱選手

    if (isBadBoard || position === '觀察期') {
      // 盤面不對勁或觀察期：直接取第3-7名（完全跳過最熱的）
      const rank3to7 = allCandidates.slice(2, 7).map(s => s.n).sort((a, b) => a - b);
      top5 = rank3to7.length >= 3 ? rank3to7 : allCandidates.slice(0, 5).map(s => s.n).sort((a, b) => a - b);
      console.log(`[buildBingoGroups] 非高命中時段 取第3-7名 top5=${top5.join(',')}`);

    } else {
      // 正常情況：取第1名（最強主力）+ 第3-6名（中後段起熱）
      // 跳過第2名，讓第3-6名的起熱選手有機會
      ...
      top5 = combined.length >= 3 ? combined : allCandidates.slice(0, 5).map(s => s.n).sort((a, b) => a - b);
      console.log(`[buildBingoGroups] 非高命中時段 第1名+第3-6名 top5=${top5.join(',')}`);
    }
  }
