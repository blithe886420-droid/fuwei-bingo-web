...
function scoreGroupShape(nums = [], marketProfile, strategyKey = "", marketSnapshot = {}) {
  ...
  // ✅ 中3導向：從 marketSnapshot 取熱號，號碼集中在熱區加分
  const hot5 = Array.isArray(marketSnapshot?.hot_5_numbers) ? marketSnapshot.hot_5_numbers.map(String) : [];
  const hot10 = Array.isArray(marketSnapshot?.hot_10_numbers) ? marketSnapshot.hot_10_numbers.map(String) : [];
  const streak3 = Array.isArray(marketSnapshot?.streak3) ? marketSnapshot.streak3.map(String) : [];
  const streak4 = Array.isArray(marketSnapshot?.streak4) ? marketSnapshot.streak4.map(String) : [];
...
function buildHotChase(todayRows, recent20, historyRows, optimizerWeight, marketProfile, marketSnapshot = {}) {
  const hot5 = Array.isArray(marketSnapshot?.hot_5_numbers) ? marketSnapshot.hot_5_numbers : [];
  const hot10 = Array.isArray(marketSnapshot?.hot_10_numbers) ? marketSnapshot.hot_10_numbers : [];
...
function buildRebound(todayRows, recent20, historyRows, optimizerWeight, marketProfile, marketSnapshot = {}) {
  const marketPhase = String(marketSnapshot?.market_phase || 'rotation').toLowerCase();
  const gapNumbers = Array.isArray(marketSnapshot?.gap_numbers) ? marketSnapshot.gap_numbers : [];
  const coldNumbers = Array.isArray(marketSnapshot?.cold_numbers) ? marketSnapshot.cold_numbers : [];
...
function buildZoneBalanced(todayRows, recent20, historyRows, optimizerWeight, marketProfile, marketSnapshot = {}) {
  const marketPhase = String(marketSnapshot?.market_phase || 'rotation').toLowerCase();
  const hot5 = Array.isArray(marketSnapshot?.hot_5_numbers) ? marketSnapshot.hot_5_numbers : [];
  const hot20 = Array.isArray(marketSnapshot?.hot_20_numbers) ? marketSnapshot.hot_20_numbers : [];
...
function buildPatternStructure(todayRows, recent20, historyRows, optimizerWeight, marketProfile, marketSnapshot = {}) {
  const marketPhase = String(marketSnapshot?.market_phase || 'rotation').toLowerCase();
  const hot5 = Array.isArray(marketSnapshot?.hot_5_numbers) ? marketSnapshot.hot_5_numbers : [];
  const streak2 = Array.isArray(marketSnapshot?.streak2) ? marketSnapshot.streak2 : [];
...
function dedupeStrategies(strategies = [], sortedCandidates = [], marketProfile, numCount = 4) {
  ...
  return strategies.map((s, idx) => {
    ...
    if (nums.length < numCount || used.has(key)) {
...
function buildStreakChase(todayRows, recent20, historyRows, optimizerWeight, marketProfile, marketSnapshot = {}) {
  const streak3 = Array.isArray(marketSnapshot?.streak3) ? marketSnapshot.streak3 : [];
  const streak4 = Array.isArray(marketSnapshot?.streak4) ? marketSnapshot.streak4 : [];
  const streak2 = Array.isArray(marketSnapshot?.streak2) ? marketSnapshot.streak2 : [];
  const attackCore = Array.isArray(marketSnapshot?.decision_basis?.attack_core_numbers)
...
// ✅ 新增：根據近10期 strategy_stats 動態決定每個策略出幾組
function decideGroupCountByPerformance(recent10Stats = {}, marketSnapshot = {}) {
  ...
  // 固定策略清單（5個），totalGroups 最少要 >= 策略數量
  const strategies = [
    { key: 'streak_chase',      roi10: recent10Stats.streak_chase      ?? -0.5 },
    { key: 'hot_chase',         roi10: recent10Stats.hot_chase          ?? -0.5 },
    { key: 'pattern_structure', roi10: recent10Stats.pattern_structure  ?? -0.5 },
    { key: 'rebound',           roi10: recent10Stats.rebound            ?? -0.5 },
    { key: 'zone_balanced',     roi10: recent10Stats.zone_balanced      ?? -0.5 }
  ].sort((a, b) => b.roi10 - a.roi10);

  const BASE_STRATEGY_COUNT = strategies.length; // 5

  // 基礎組數：根據盤面強度決定總組數
  // ✅ 修正：最少必須 >= BASE_STRATEGY_COUNT（5），避免 remaining 為負
  let totalGroups;
  if (streak4Count >= 2 || (streak3Count >= 3 && marketPhase === 'continuation')) {
    totalGroups = 8; // 強延續盤，最大火力
  } else if (streak3Count >= 2 || marketPhase === 'continuation') {
    totalGroups = 7; // 中強盤
  } else if (streak3Count >= 1 || marketPhase === 'weak_trend') {
    totalGroups = 6; // 弱趨勢
  } else {
    totalGroups = 5; // 輪動盤：每個策略各出1組
  }
  ...
  // 剩餘組數依 hit3 分數分配：
  // - 分數最高的策略最多可拿3組額外（中3率高就多給機會）
  // - 分數最低的（hit3=0）只維持1組
  let extra = totalGroups - BASE_STRATEGY_COUNT; // 保證 >= 0
  for (const s of strategies) {
    if (extra <= 0) break;
    // 分數 > 0（有中3記錄）最多加3組，否則最多加1組
    const maxAdd = s.roi10 > 0 ? 3 : 1;
