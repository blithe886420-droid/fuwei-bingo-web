/**
 * buildBingoV1Strategies.js - V0614-11
 *
 * 蜘蛛感知系統 + 六層觸發條件選號策略
 *
...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  const triggerConsec8 = prevConsecCount >= 8;
  const triggerBalanced2 = prevIsBalanced && prev2IsBalanced; // 新：連續2期均衡→+23.08(26筆)/最佳組合230筆-24
  ...
  const combos_consec8 = [
    [1,2,4],[2,6,9],[2,4,9],[4,6,9],[2,4,6],[6,8,9],[1,2,3],[3,4,6],
  ];
  const combos_balanced2 = [  // 連續2期均衡：230筆，排名8/9為核心
    [4,8,9],[4,6,9],[2,3,8],[6,8,9],[4,5,8],[6,7,9],[2,4,8],[7,8,9],
  ];
  ...
  const activeCombos = triggerBurstBrew ? combos_burstBrew
    ...
    : triggerBalanced2 ? combos_balanced2
  ...
  const activeMode = triggerBurstBrew ? 'burst_brew'
    ...
    : triggerBalanced2 ? 'balanced2'
    : triggerOddOdd ? 'odd_odd'
    : 'standard';
  console.log(`[buildBingoGroups] mode=${activeMode} changed=${changedNums} TQ=${totalQualified} pos=${position} prevHighZone=${prevHighZoneCount} prevSum=${prevSumVal} prevConsec=${prevConsecCount} prevOddTail=${prevOddTail} 組數=${finalCombos.length}`);
  ...
  return finalCombos.map(combo => {
    ...
    return {
      ...
      meta: {
        ...
        spider_sense_active: spiderSenseActive || triggerBurstBrew || triggerSlowRich || triggerHighZone || triggerHighZoneRich || triggerTailSlow || triggerLossFast || triggerOddOdd || triggerConsec8 || triggerBalanced2,
