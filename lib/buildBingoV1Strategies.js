...
export function buildBingoV1Strategies(allRows = [], strategyWeightMap = {}, starCount = 4, marketSnapshot = {}, recent10Stats = {}, dynamicStrategyKeys = [], forcedGroupCount = null) {
  ...
  // 每期根據當下盤面狀況（連號、冷號、熱區集中度等）靈活調整選號組合
  ...
  if (starCount === 3 && marketCondition && dynamicRoles) {
    // 三星：根據盤面動態角色逐組選號，確保每組號碼不重疊
    // ✅ 修復：傳入 marketSnapshot，讓 buildNumsByRole 能使用 freq20_hot_nums
    ...
    dynamicRoles.forEach((role, idx) => {
      ...
      rawStrategies.push({
        ...
        nums,
        reason: `盤面感知[${role}] 連號:${marketCondition.streakStrength} 熱區:${marketCondition.hotConcentration} 冷號:${marketCondition.coldPressure}`,
        meta: {
