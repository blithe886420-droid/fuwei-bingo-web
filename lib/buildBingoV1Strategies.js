...
function decideDynamicRoleAllocation(condition = {}, totalGroups = 8) {
  ...
  if (marketPhase === 'continuation') {
    // ✅ 步驟五反饋：rebound(3.2%) > recent(2.0%) > mix_zone(1.6%) 在continuation盤最佳
    // 增加 cold(rebound邏輯) 和 recent，減少純 hot 集中
    if (streakStrength >= 5) {
      roles = ['streak', 'cold', 'recent', 'cold', 'hot', 'zone_fill', 'scatter', 'streak'];
    } else {
      roles = ['cold', 'recent', 'cold', 'hot', 'streak', 'zone_fill', 'scatter', 'recent'];
    }

  } else if (marketPhase === 'bias') {
    // ✅ 步驟五反饋：cold_zone(2.0%) > board_6(2.0%) > rebound(1.0%) 在bias盤最佳
    // 增加 cold 和 zone_fill，因為 bias 盤有區段集中，冷號回補效果好
    if (zoneBias >= 25) {
      roles = ['cold', 'zone_fill', 'cold', 'zone_fill', 'hot', 'recent', 'streak', 'scatter'];
    } else {
      roles = ['cold', 'zone_fill', 'cold', 'hot', 'recent', 'streak', 'zone_fill', 'scatter'];
    }
