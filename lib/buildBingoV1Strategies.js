...
export function buildBingoV1Strategies(allRows = [], strategyWeightMap = {}, starCount = 4, marketSnapshot = {}, recent10Stats = {}, dynamicStrategyKeys = [], forcedGroupCount = null) {
  ...
  // ✅ 修正一：強制熱區覆蓋組
  // 近期開獎平均命中只有 4~6 個（理論應是6個），代表選號跟開獎熱區重疊不足
  // 強制產生2組「純熱區覆蓋」組，確保跟開獎熱區重疊
  // 這2組直接從近期最熱的號碼裡選，不走策略邏輯
  function buildHotZoneCoverGroup(recentRows, usedNums, groupIdx) {
    // 近5期開獎最常出現的號碼
    const freq5 = {};
    ...
    // 近10期加權
    recentRows.slice(0, 10).forEach(row => {
      parseNumbers(row.numbers).forEach(n => {
        freq5[n] = (freq5[n] || 0) + 1;
      });
    });
    ...
    // 按分數排序，取未使用的號碼
    const sorted = Object.entries(freq5)
      .sort((a, b) => b[1] - a[1])
      .map(([n]) => Number(n))
      .filter(n => !usedNums.has(n) && n >= 1 && n <= 80);
    ...
    for (const n of sorted) {
      ...
      const tail = n % 10;
      // 優先選尾數不重複的
      if (!pickedTails.has(tail) || picked.length < 2) {
        picked.push(n);
  ...
  // ✅ 三星模式：強制插入2組熱區覆蓋組（取代最後2個策略組）
  // 確保每期至少有2組號碼跟開獎熱區高度重疊
  if (starCount === 3 && rawStrategies.length >= 3) {
    // 用臨時 usedNums 計算前6組已用的號碼
    const tempUsed = new Set();
    rawStrategies.slice(0, Math.min(6, rawStrategies.length)).forEach(s => {
      (s.nums || []).forEach(n => tempUsed.add(Number(n)));
    });
    // 替換最後2組為熱區覆蓋組
    rawStrategies.splice(rawStrategies.length - 2, 2,
      buildHotZoneCoverGroup(allRows, tempUsed, 1),
      buildHotZoneCoverGroup(allRows, new Set([...tempUsed, ...(rawStrategies[rawStrategies.length-2]?.nums || []).map(Number)]), 2)
    );
  }
