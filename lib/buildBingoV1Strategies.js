...
export function buildBingoGroups(recentDraws = [], latestDrawNo = 0, recentPredictions = []) {
  ...
  // ★ V0614-2：蜘蛛感知選號邏輯。
  // 觸發條件：TQ>=22(合格池豐富) + 連續換手穩定2期(本期+上期)
  // 驗證：4天351期，觸發頻率每日平均2.5次，觸發條件下avg_pnl改善。
  // 擴展候選池到12名，後2組換成含11-12名的擴展位置組合。
  // 上期換手穩定判斷：用prevHotPool計算上期換手數(changedNums)，
  // 但changedNums是「本期vs上期」，這裡另外計算「上期vs上上期」。
  ...
  const spiderSenseActive = totalQualified >= 22 && isSlowTurnover && prevIsSlowTurnover;

  const hotPool10Ranked = allCandidates.slice(0, 10).map(s => s.n);
  const hotPool12Ranked = allCandidates.slice(0, 12).map(s => s.n);
  const hotPool7 = [...hotPool10Ranked.slice(0, 7)].sort((a, b) => a - b); // 供hot_pool欄位顯示

  // 標準G策略8組(前10名)
  const standardCombos = [
    [1, 5, 8], [1, 5, 10], [4, 5, 8], [1, 4, 5],
    [4, 5, 7], [3, 5, 10], [2, 4, 7], [2, 4, 10],
  ];

  // 蜘蛛感知擴展8組(前12名，後2組換成含11-12名的跨段組合)
  const spiderCombos = [
    [1, 5, 8], [1, 5, 10], [4, 5, 8], [1, 4, 5],
    [4, 5, 7], [3, 5, 10], [2, 4, 11], [2, 4, 12],
  ];

  const activePool = spiderSenseActive ? hotPool12Ranked : hotPool10Ranked;
  const activeCombos = spiderSenseActive ? spiderCombos : standardCombos;
  const requiredPoolSize = spiderSenseActive ? 12 : 10;

  const finalCombos = activePool.length === requiredPoolSize
    ? activeCombos.map(([i, j, k]) =>
        [activePool[i - 1], activePool[j - 1], activePool[k - 1]].sort((a, b) => a - b)
      )
    : [];

  console.log(`[buildBingoGroups] spiderMode=${spiderMode} spiderSenseActive=${spiderSenseActive} TQ=${totalQualified} top5=${top5.join(',')} hotPool=${spiderSenseActive ? hotPool12Ranked.join(',') : hotPool10Ranked.join(',')} 真信號=${trueSignalCount} 組數=${finalCombos.length}`);
  ...
  return finalCombos.map(combo => {
    ...
    return {
      ...
      meta: {
        ...
        spider_sense_active: spiderSenseActive,
