...
function scoreGroupShape(nums = [], marketProfile, strategyKey = "", marketSnapshot = {}) {
  ...
  if (streak3Hits + streak4Hits >= 2) score += 6.0; // 連號仍保留（有市場依據）
...
// ✅ v15：前3組為「強訊號加碼組」，允許號碼重疊（強化最強策略的押注）
//         後5組為「覆蓋補充組」，維持原本去重邏輯，最大化覆蓋廣度
// 只對重複「整組完全相同」的組合做防護，避免完全一樣的組出現兩次
const SIGNAL_BOOST_GROUPS = 3; // 前N組允許重疊

function dedupeStrategies(strategies = [], sortedCandidates = [], marketProfile, numCount = 4) {
  const usedNums = new Set();   // 後5組去重用：已使用號碼
  const usedCombos = new Set(); // 全域防護：避免完全重複的組合
  ...
  return strategies.map((s, idx) => {
    ...
    if (idx < SIGNAL_BOOST_GROUPS) {
      // ✅ 前3組：強訊號加碼組，只防「完全相同組合」，允許號碼重疊
      if (isDuplicate || nums.length < numCount) {
        // 組合完全重複時，微調一顆號碼使其不同
        const allUnused = [];
        for (const candidate of sortedCandidates) {
          const n = Number(candidate.num);
          if (!Number.isFinite(n) || n < 1 || n > 80) continue;
          if (!nums.includes(n)) allUnused.push(n);
          if (allUnused.length >= numCount) break;
        }
        // 替換最後一顆號碼
        if (allUnused.length > 0) {
          nums = [...nums.slice(0, numCount - 1), allUnused[0]];
        }
      }
      // 前3組不寫入 usedNums，不參與後5組的去重計算
...
function decideDynamicRoleAllocation(condition = {}, totalGroups = 8) {
  ...
  console.log(`[role_alloc] phase=${marketPhase} streak=${streakStrength} hotConc=${hotConcentration} → ${roles.slice(0, totalGroups).join(',')}`);
...
export function buildBingoV1Strategies(allRows = [], strategyWeightMap = {}, starCount = 4, marketSnapshot = {}, recent10Stats = {}, dynamicStrategyKeys = [], forcedGroupCount = null) {
  ...
  if (starCount === 3 && deduped.length >= 3) {
    ...
    deduped[deduped.length - 1] = {
      key: 'hot_zone_cover_2',
      label: '熱區覆蓋2',
      groupNo: deduped.length,
      nums: hotNums2,
      reason: '強制熱區覆蓋：近期開獎高頻號碼（第2組）',
      meta: { model: 'hot_zone_cover', focus: 'coverage_boost', star_mode: 3 }
    };
  }
