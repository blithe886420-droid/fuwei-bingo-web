...
function extractStrategyRowsFromGroups(
  groups = [],
  defaultSourceType = 'seed',
  ...
  return groups
    .map((group) => {
      const strategyKey = normalizeStrategyKey(
        group?.meta?.strategy_key || group?.strategyKey || group?.key || ''
      );

      if (!strategyKey) return null;

      const inferred = inferGenes(strategyKey);

      const geneA =
        String(group?.meta?.gene_a || '')
          .trim()
          .toLowerCase() || inferred.gene_a;

      const geneB =
        String(group?.meta?.gene_b || '')
          .trim()
          .toLowerCase() || inferred.gene_b;
      ...
      return {
        ...
        strategy_name:
          ...
          strategyKey,
        gene_a: KNOWN_GENES.has(geneA) ? geneA : inferred.gene_a,
        gene_b: KNOWN_GENES.has(geneB) ? geneB : inferred.gene_b,
        source_type: safeSourceType(
          group?.meta?.source_type || group?.meta?.source || defaultSourceType
        ),
...
export async function ensureStrategyPoolStrategies({
  ...
  const groupRows = extractStrategyRowsFromGroups(
    groups,
    finalSourceType,
    finalStatus
  );
  ...
  const existingMap = new Map(
    (existingRows || []).map((row) => [row.strategy_key, row])
  );
  ...
  for (const row of finalRows) {
    ...
    if (
      existing.strategy_name !== (row.strategy_name || existing.strategy_name)
    ) {
      patch.strategy_name = row.strategy_name || existing.strategy_name;
      changed = true;
    }

    if (
      (!existing.gene_a ||
        !KNOWN_GENES.has(String(existing.gene_a).toLowerCase())) &&
      row.gene_a
    ) {
    ...
    if (
      (!existing.gene_b ||
        !KNOWN_GENES.has(String(existing.gene_b).toLowerCase())) &&
      row.gene_b
    ) {
