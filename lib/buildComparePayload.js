...
export function buildComparePayload({
  ...
  const normalizedDrawRows = (Array.isArray(drawRows) ? drawRows : [])
    .map((row) => ({
  ...
  const appResults = groupsResult.map((g) => ({
    key: g.key,
    label: g.label,
    strategyKey: g.meta?.strategy_key || g.key,
    nums: g.nums,
    hitCount: g.total_hit_count,
    totalHitCount: g.total_hit_count,
    ...
    roi: g.roi,
    hit0Count: g.hit0_count,
    hit1Count: g.hit1_count,
