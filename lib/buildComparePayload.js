...
function buildGroupSummary(group, normalizedDrawRows, costPerGroupPerPeriod) {
  ...
  let hit3Count = 0;
  let hit4Count = 0;
  let groupBestSingleHit = 0;

  const rounds = normalizedDrawRows.map((drawRow) => {
    const round = buildRoundResult(group, drawRow, costPerGroupPerPeriod);

    totalCost += round.cost;
    totalReward += round.reward;
    totalProfit += round.profit;
    totalHitCount += round.hit_count;

    if (round.hit_count === 0) hit0Count += 1;
    else if (round.hit_count === 1) hit1Count += 1;
    else if (round.hit_count === 2) hit2Count += 1;
    else if (round.hit_count === 3) hit3Count += 1;
    else if (round.hit_count >= 4) hit4Count += 1;

    groupBestSingleHit = Math.max(groupBestSingleHit, round.hit_count);
  ...
  const profitWinRate =
    rounds.length > 0
      ? (rounds.filter((r) => r.profit > 0).length / rounds.length) * 100
      : 0;
  ...
  return {
    ...
    best_single_hit: groupBestSingleHit,
...
export function buildComparePayload({
  ...
  const overallTotalCost = groupsResult.reduce((sum, g) => sum + toInt(g.total_cost, 0), 0);
  const overallTotalReward = groupsResult.reduce((sum, g) => sum + toInt(g.total_reward, 0), 0);
  const overallTotalProfit = groupsResult.reduce((sum, g) => sum + toInt(g.total_profit, 0), 0);
  const overallTotalHitCount = groupsResult.reduce((sum, g) => sum + toInt(g.total_hit_count, 0), 0);
  const bestSingleHit = groupsResult.reduce((max, g) => Math.max(max, toInt(g.best_single_hit, 0)), 0);
  ...
  const appGroups = groupsResult.map((g) => ({
    key: g.key,
    label: g.label,
    nums: g.nums,
    reason: g.reason || '',
