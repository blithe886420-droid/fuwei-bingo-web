...
export function buildComparePayload({
  ...
  for (const draw of safeDrawRows) {
    ...
    for (const group of safeGroups) {
      ...
      const reward = calcReward(hit, starMode);
      ...
      const row = {
        ...
        reward,
        nums: group.nums,
        draw_nums: nums   // ★ 新增：該期開獎號碼，前端球高亮用
      };
  ...
  const coverageCount = allCoveredNums.length;
  ...
  // ★ 取第一筆 draw 的開獎號碼存到最外層（單期比對固定只有一筆 draw）
  const topLevelDrawNums = detail.length > 0 ? (detail[0].draw_nums || []) : [];
  ...
  return {
    ...
    compareResult: {
      ...
      total_profit: totalProfit,
      roi: round4(roi),
      draw_nums: topLevelDrawNums,           // ★ 新增：比對期開獎號碼，前端球高亮用
      coverage_nums: allCoveredNums,
      coverage_count: coverageCount,
      coverage_hit_per_draw: coverageHitPerDraw,
      avg_coverage_hit: avgCoverageHit
    }
  };
}
