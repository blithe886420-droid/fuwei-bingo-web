export function parseDrawNumbers(value) {
  if (Array.isArray(value)) return value.map(Number).filter(Number.isFinite);

  if (typeof value === 'string') {
    return value
      .split(/[,\s]+/)
      .map((s) => Number(String(s).trim()))
      .filter(Number.isFinite);
  }

  return [];
}

export function parsePredictionGroups(prediction, limit = 4) {
  const groups = Array.isArray(prediction?.groups_json)
    ? prediction.groups_json
    : [];

  return groups.slice(0, limit).map((g, idx) => ({
    key: g.key || `g_${idx}`,
    nums: (g.nums || []).map(Number).filter(Number.isFinite)
  }));
}

function calcReward(hit) {
  if (hit >= 4) return 450;
  if (hit === 3) return 50;
  if (hit === 2) return 10;
  return 0;
}

export function buildComparePayload({
  prediction,
  groups,
  drawRows,
  drawNumbersCol = 'numbers',
  costPerGroupPerPeriod = 25
}) {
  let totalHit = 0;
  let totalCost = 0;
  let totalReward = 0;

  const detail = [];

  for (const draw of drawRows) {
    const nums = parseDrawNumbers(draw[drawNumbersCol]);

    for (const g of groups) {
      const hit = g.nums.filter((n) => nums.includes(n)).length;

      const reward = calcReward(hit);
      const cost = costPerGroupPerPeriod;

      totalHit += hit;
      totalCost += cost;
      totalReward += reward;

      detail.push({
        strategy_key: g.key,
        hit,
        reward,
        cost
      });
    }
  }

  const totalProfit = totalReward - totalCost;
  const roi = totalCost > 0 ? totalProfit / totalCost : 0;

  return {
    hitCount: totalHit,
    verdict: totalProfit > 0 ? 'good' : 'bad',
    compareResult: {
      detail,
      total_hit: totalHit,
      total_cost: totalCost,
      total_reward: totalReward,
      total_profit: totalProfit,
      roi
    }
  };
}
