 ...
 * 3. 保留真正有 hit2 / 近期 hit3 爆發能力的策略（三星不可能hit4）
 *
 * D版補強：
...
function calcPhaseBestJson(phaseStatsJson = {}) {
  ...
  for (const phase of VALID_PHASES) {
    ...
    // ✅ 三星化：移除 hit4_rate 和 recent_20_hit4_rate 權重（三星不可能hit4）
    // avg_hit 理論值 0.75，hit2_rate 理論 14%，hit3_rate 理論 1.06%
    const score =
      bucket.avg_hit * 80 +           // 降低（三星avg_hit理論才0.75）
      bucket.hit2_rate * 320 +        // 加重（三星hit2是主要回血）
      bucket.hit3_rate * 500 +        // hit3最重要
      // bucket.hit4_rate * 700 +     // 三星不可能hit4，移除
      bucket.recent_20_hit_rate * 220 +
      bucket.recent_20_hit3_rate * 380 +
      // bucket.recent_20_hit4_rate * 520 + // 三星不可能hit4，移除
      bucket.recent_20_roi * 60 +
...
function shouldDisableStrategy(row = {}) {
  ...
  if (totalRounds >= DISABLE_RULES.minRoundsJ && hit2Rate < DISABLE_RULES.hit2RateCeilJ && hit3Rate < DISABLE_RULES.hit3RateFloorJ && recent50Roi < DISABLE_RULES.recent50RoiFloorJ) return { shouldDisable: true, reason: `low_hit2_rate_below_${DISABLE_RULES.hit2RateCeilJ}_and_low_hit3_rate_and_recent50_roi_below_${DISABLE_RULES.recent50RoiFloorJ}` };
...
// ✅ v18 第五階段：計算各因子與命中率的相關性
// 用於判斷哪個因子對中3最有幫助，為第六階段自動調整權重做準備
function calcFactorCorrelation(history = []) {
  if (!Array.isArray(history) || history.length < 5) {
    return { hot: 0, overdue: 0, streak: 0, cold: 0, freq: 0, sample_size: 0 };
  }

  const factors = ['hot_score', 'overdue_score', 'streak_score', 'cold_score', 'freq_score'];
  const result = { sample_size: history.length };

  for (const factor of factors) {
    const pairs = history
      .filter(h => h && typeof h[factor] === 'number' && typeof h.hit_result === 'number')
      .map(h => ({ x: h[factor], y: h.hit_result >= 3 ? 1 : 0 }));

    if (pairs.length < 3) {
      result[factor.replace('_score', '')] = 0;
      continue;
    }
    ...
    result[factor.replace('_score', '')] = den > 0 ? round4(num / den) : 0;
  }
...
export async function recordStrategyCompareResult(compareResult) {
  ...
  // ✅ v14：只記錄 formal_3star 模式，test/formal 模式不寫入
  // auto-train 傳入 star_mode=3，prediction-save 傳入 mode='formal_3star'
  const isFormal3Star = compareResult?.mode === 'formal_3star' || compareResult?.star_mode === 3;
  if (!isFormal3Star) {
    return { ok: true, skipped: true, reason: 'non_formal_3star_mode' };
  }
  ...
  // ✅ v10：同時收集 strategy_name（含前綴），用於以 strategy_name 為主鍵的統計
  const uniqueNames = [...new Set(detail.map((row) => String(row?.strategy_name || row?.strategy_key || '').trim()).filter(Boolean))];
  ...
  // ✅ v10：改用 strategy_name（含前綴如HOT|mix_gap）為統計主鍵
  // 同一 strategy_key 不同前綴視為不同策略，各自累積統計
  // 正確做法：同一 draw_no 同一 strategy_name，只記一次 hit（取最高命中數）
  const drawNo = detail[0]?.draw_no || 0;
  const bestHitPerStrategy = new Map(); // key = strategy_name
  const bestCoveragePerStrategy = new Map();
  for (const row of detail) {
    // ✅ v10：用 strategy_name（含前綴）為 key
    const k = String(row?.strategy_name || row?.strategy_key || '').trim();
    if (!k) continue;
    const h = toNum(row?.hit, 0);
    const cov = toNum(row?.coverage_hit, 0);
    if (!bestHitPerStrategy.has(k) || h > bestHitPerStrategy.get(k)) {
      bestHitPerStrategy.set(k, h);
    }
    if (!bestCoveragePerStrategy.has(k) || cov > bestCoveragePerStrategy.get(k)) {
      bestCoveragePerStrategy.set(k, cov);
    }
  }

  // ✅ v10：用去重後的 strategy_name 清單（每個 strategy_name 只處理一次）
  const processedKeys = new Set();

  for (const row of detail) {
    // ✅ v10：主鍵改為 strategy_name（含前綴，如 HOT|mix_gap）
    const nameKey = String(row?.strategy_name || row?.strategy_key || '').trim();
    const key = String(row?.strategy_key || '').trim();
    if (!nameKey) {
      console.warn('[strategyStatsRecorder] strategy_name missing, skipping row:', JSON.stringify(row));
      continue;
    }
    if (processedKeys.has(nameKey)) continue;
    processedKeys.add(nameKey);

    // ✅ strategy_pool 仍用 strategy_key 查詢狀態
    ...
    // ✅ v10：用 strategy_name 取最高命中數
    const hit = bestHitPerStrategy.get(nameKey) ?? toNum(row?.hit, 0);
    ...
    // ✅ v10：用 strategy_name（含前綴）查詢和寫入，HOT|mix_gap 和 COLD|mix_gap 分開統計
    const { data: old, error: oldError } = await supabase
      .from(STRATEGY_STATS_TABLE)
      .select('*')
      .eq('strategy_name', nameKey)
      .maybeSingle();

    if (oldError) {
      console.warn(`[strategyStatsRecorder] strategy_stats select failed for ${nameKey}, skipping:`, oldError.message);
      continue;
    }
    ...
    // ✅ v10：coverageHit 也改用 nameKey
    const coverageHit = bestCoveragePerStrategy.get(nameKey) ?? toNum(row?.coverage_hit, 0);
    ...
    // ✅ v18 第五階段：記錄選號因子分數（為權重自動學習建立基礎數據）
    // 從 compareResult 的 meta 裡取出 boardState 的因子分數
    // 這樣之後可以用「哪個因子貢獻最多 → 中了沒」來反推最優權重
    const boardMeta = compareResult?.board_meta || {};
    const factorScores = {
      hot_score: toNum(boardMeta?.hot_score, 0),
      overdue_score: toNum(boardMeta?.overdue_score, 0),
      streak_score: toNum(boardMeta?.streak_score, 0),
      cold_score: toNum(boardMeta?.cold_score, 0),
      freq_score: toNum(boardMeta?.freq_score, 0),
      market_phase: boardMeta?.market_phase || marketPhase,
      hit_result: hit,
      draw_no: toNum(row?.draw_no || compareResult?.draw_no, 0)
    };

    // 累積近50期的因子分數記錄
    const oldFactorHistory = safeArrayValue(old?.factor_scores_history);
    const factor_scores_history = tail([...oldFactorHistory, factorScores], 50);

    // 計算各因子的命中相關性（因子分高時，中3率是否也高）
    const factorCorrelation = calcFactorCorrelation(factor_scores_history);

    const payload = {
      strategy_name: nameKey,   // ✅ v10：主鍵改為 strategy_name（含前綴）
      strategy_key: key,        // ✅ 保留原始 key 供參考
      total_rounds,
      total_hits,
      total_cost,
      total_reward,
    ...
    const { error: upsertError } = await supabase
      .from(STRATEGY_STATS_TABLE)
      .upsert(payload, { onConflict: 'strategy_name' }); // ✅ v10：衝突鍵改為 strategy_name

    if (upsertError) {
      console.warn(`[strategyStatsRecorder] strategy_stats upsert failed for ${nameKey}, skipping:`, upsertError.message);
      continue;
    }

    updatedKeys.push(nameKey);
    ...
    if (is3starMode) {
      // ✅ 修正二：三星淘汰條件補強（保守版）
      // 條件A：30期後 hit2=0 且 hit3=0（原有條件）
      const cond_A = payload.total_rounds >= 30 &&
        payload.hit2 === 0 && payload.hit3 === 0;

      // 條件B：50期後 hit2率低於3% 且 ROI極差（理論14%，低於3%屬嚴重異常）
      const cond_B = payload.total_rounds >= 50 &&
        payload.hit2_rate < 0.03 &&
        payload.recent_50_roi < -0.80;

      // 條件C：40期後近50期 hit2率低於4% 且 hit3=0 且 ROI很差
      const cond_C = payload.total_rounds >= 40 &&
        payload.recent_50_hit_rate < 0.04 &&
        payload.recent_50_hit3_rate <= 0 &&
        payload.recent_50_roi < -0.75;

      if (cond_A || cond_B || cond_C) {
        const reason = cond_A ? 'three_star_no_hit2_after_30'
          : cond_B ? 'three_star_hit2_rate_below_3pct_after_50'
          : 'three_star_recent_hit2_below_4pct_no_hit3_after_40';
        disabledKeys.push(key);
        disabledReasonMap[key] = reason;
      }
      continue;
    }
