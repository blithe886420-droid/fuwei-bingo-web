...
function calcStableScore({
  ...
  const roiPart = roiClamped * 42;
  const avgHitPart = avgHitDelta * 60;
  const recentRoiPart = recentRoiClamped * 28;
  const recentHitRatePart = hitRateClamped * 18;
  ...
  const qualityRateBonus =
    clamp(hit2_rate, 0, 1) * 18 +
    hit3RateClamped * 560 +
    hit4RateClamped * 1200 +
    recentHit3RateClamped * 620 +
    recentHit4RateClamped * 900;
