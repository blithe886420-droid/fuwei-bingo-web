...
const VALID_PHASES = ['continuation', 'rotation', 'bias', 'chaos', 'hot_bias', 'hot_streak'];  // ✅ v2：補入新盤相
...
function calcStableScore({
  ...
  // ✅ v3：降低 ROI 權重（原10/8），避免長期累積的 ROI 讓老策略永遠高分
  // 提高 hit3_rate 的直接影響，讓真正有效的策略能快速浮現
  const roiPart = roiClamped * 3;        // 原10，大幅降低
  const recentRoiPart = recentRoiClamped * 2;  // 原8，大幅降低
  const avgHitPart = avgHitDelta * 70;
  const roundsBonus = roundsClamped * 0.3;  // 原0.8，降低期數加成，避免老策略靠期數堆分
  ...
  // ✅ v3：降低 countPart，避免長期策略靠累積次數堆分
  const countPart = toNum(hit2, 0) * 1.0 + toNum(hit3, 0) * 6;  // 原 hit2*2 + hit3*12
...
export async function recordStrategyCompareResult(compareResult) {
  ...
  for (const row of detail) {
    ...
    // ✅ 第五階段 v1：因子表現記錄（完全獨立，失敗不影響主流程）
    try {
      const factorPayload = {
        strategy_name: nameKey,
        strategy_key: key,
        draw_no: lastResultDrawNo,
        recorded_at: nowIso,
        hit,
        hit2: hit2Add,
        hit3: hit3Add,
        avg_hit_at_time: round4(avg_hit),
        recent_50_roi_at_time: round4(recent_50_roi),
        recent_50_hit3_rate_at_time: round4(recent_50_hit3_rate),
        avg_coverage_hit_at_time: round4(avg_coverage_hit),
        recent_coverage_hit_rate_at_time: round4(recent_coverage_hit_rate),
        market_phase: marketPhase,
        score_at_time: score,
        total_rounds_at_time: total_rounds
      };
      const { error: factorError } = await supabase
        .from('strategy_factor_stats')
        .insert(factorPayload);
      if (factorError) {
        console.warn(`[phase5] factor insert failed for ${nameKey}:`, factorError.message);
      }
    } catch (factorEx) {
      console.warn(`[phase5] factor insert exception for ${nameKey}:`, factorEx.message);
    }
