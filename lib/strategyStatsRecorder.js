...
function calcStableScore({
   ...
   * 分數升級方向：
   * 1. ROI 仍保留，但不再主宰總分
   * 2. avg_hit / hit3 / hit4 / 近期 hit3 / 近期 hit4 成為核心
   * 3. 對「近期有變強」的策略給明顯加分，避免一直卡在 50 附近
   */
  const roiPart = roiClamped * 18;
  const recentRoiPart = recentRoiClamped * 12;
  const avgHitPart = avgHitDelta * 135;
  const recentHitRatePart = hitRateClamped * 52;
  const roundsBonus = roundsClamped * 1.15;
  ...
  const qualityRateBonus =
    hit2RateClamped * 32 +
    hit3RateClamped * 760 +
    hit4RateClamped * 1450 +
    recentHit3RateClamped * 900 +
    recentHit4RateClamped * 1280;

  let marketAdjustment = 0;

  if (avg_hit >= 1.2) marketAdjustment += 65;
  if (avg_hit >= 1.35) marketAdjustment += 40;
  if (hitRateClamped >= 0.3) marketAdjustment += 35;
  if (hitRateClamped >= 0.4) marketAdjustment += 30;
  if (hit3RateClamped >= 0.08) marketAdjustment += 95;
  if (hit3RateClamped >= 0.12) marketAdjustment += 65;
  if (recentHit3RateClamped >= 0.06) marketAdjustment += 110;
  if (recentHit3RateClamped >= 0.1) marketAdjustment += 70;
  if (hit4RateClamped >= 0.01) marketAdjustment += 120;
  if (recentHit4RateClamped >= 0.01) marketAdjustment += 140;

  /**
   * 若近期命中品質明顯提升，抵銷部分負 ROI 壓制。
   */
  if (roi < 0 && (hit3RateClamped >= 0.08 || recentHit3RateClamped >= 0.06)) {
    marketAdjustment += 45;
  }

  if (recent_50_roi < -0.5 && recentHit3RateClamped <= 0.01 && recentHit4RateClamped <= 0) {
    marketAdjustment -= 40;
  }
...
function shouldDisableStrategy(row = {}) {
   ...
   */
  if (hit4Rate >= 0.01 || recent50Hit3Rate >= 0.08 || hit3Rate >= 0.10 || avgHit >= 1.45) {
    return {
      shouldDisable: false,
      reason: ''
    };
