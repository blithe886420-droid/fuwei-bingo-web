 ...
 */
const DISABLE_RULES = {
  // A: 整體 ROI 太低（放寬：輪數提高到 15，門檻放寬到 -0.7）
  minRoundsA: 15,
  roiFloorA: -0.7,
  // B: 近期 ROI 差且命中低（放寬：輪數提高到 12，門檻放寬）
  minRoundsB: 12,
  recent50RoiFloorB: -0.5,
  avgHitFloorB: 0.8,
  // C: 近期 ROI 差且命中率低（放寬：輪數提高到 15）
  minRoundsC: 15,
  recent50RoiFloorC: -0.55,
  hitRateFloorC: 0.08,
  // D: ROI 差且平均命中低（放寬：輪數提高到 15）
  minRoundsD: 15,
  roiFloorD: -0.5,
  avgHitFloorD: 0.9,
  // E: 早期快速淘汰（放寬：輪數提高到 20，避免新策略過早死亡）
  minRoundsE: 20,
  roiFloorE: -0.7,
  // F: 早期命中太低（放寬：輪數提高到 20）
  minRoundsF: 20,
  avgHitFloorF: 0.6,
  recent50RoiFloorF: -0.6,
  // G: hit3 率低且 ROI 差（放寬：輪數提高到 20）
  minRoundsG: 20,
  hit3RateFloorG: 0.05,
  recent50RoiFloorG: -0.15,
  avgHitFloorG: 1.0,
  // H: 完全沒有 hit3/hit4（放寬：輪數提高到 20）
  minRoundsH: 20,
  hit3CountFloorH: 0,
  hit4CountFloorH: 0,
  roiFloorH: -0.1,
  // I: 近期 hit3 極低（放寬：輪數提高到 15）
  minRoundsI: 15,
  recent50Hit3RateFloorI: 0.01,
  recent50RoiFloorI: -0.15,
  recent50HitRateFloorI: 0.10,
  // J: hit2 高但 hit3 低（放寬：輪數提高到 20）
  minRoundsJ: 20,
  hit2RateCeilJ: 0.28,
  hit3RateFloorJ: 0.03,
  recent50RoiFloorJ: -0.10,
  // K: 近期命中率不足（放寬：輪數提高到 20）
  minRoundsK: 20,
  recent50HitRateFloorK: 0.12,
  recent50Hit3RateFloorK: 0.005,
  roiFloorK: -0.10,
  // L: hit3 仍是 0（放寬：輪數提高到 15）
  minRoundsL: 15,
  hit3CountFloorL: 0,
  recent50RoiFloorL: -0.10,
  // M: 近期無 hit3/hit4 且命中低（放寬：輪數提高到 20）
  minRoundsM: 20,
...
export async function recordStrategyCompareResult(compareResult) {
  ...
  for (const row of detail) {
    ...
    // ✅ last_result_draw_no：從 compareResult.draw_no 或 detail row 取得
    ...
    // ✅ 覆蓋率統計
    const coverageHit = toNum(row?.coverage_hit, 0); // 這期開獎號碼落在覆蓋範圍幾個
    ...
    // ✅ 三星模式保護：三星的 detail 裡 cost=25 reward 是三星獎金結構
    // 三星中3=500，中2=50，四星中3=100，中2=25
    // 用 reward 判斷是否為三星：三星中2 reward=50，四星中2 reward=25
    // 保護三星策略不被四星標準淘汰（三星本來就更難中）
    const is3starMode = toNum(row?.reward, 0) === 50 || toNum(row?.reward, 0) === 500;
    if (is3starMode) {
      // 三星用更寬鬆的淘汰標準：只有完全沒中2且期數夠多才淘汰
      const threeStar_shouldDisable = payload.total_rounds >= 50 && payload.hit2 === 0 && payload.hit3 === 0;
      if (threeStar_shouldDisable) {
        disabledKeys.push(key);
        disabledReasonMap[key] = 'three_star_no_hit2_after_50';
      }
      continue;
    }
