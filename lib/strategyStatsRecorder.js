 ...
 * 目標：
 * 1. 不再只看 avg_hit / roi
 * 2. 提高 hit3_rate / recent_50_hit3_rate 的生死權重
 * 3. 更早淘汰「不會中3、又沒有上限」的策略
 */
const DISABLE_RULES = {
  // A：基本 ROI 淘汰
  minRoundsA: 5,
  ...
  // G：跑一段時間，但 hit3 能力太弱 → 提前砍
  minRoundsG: 8,
  hit3RateFloorG: 0.05,
  recent50RoiFloorG: -0.1,
  avgHitFloorG: 1.15,
  ...
  recent50RoiFloorI: -0.1,
  recent50HitRateFloorI: 0.15,

  // J：有些策略只會保2，完全拉不起中3，且近期 ROI 為負
  minRoundsJ: 10,
  hit2RateCeilJ: 0.35,
  hit3RateFloorJ: 0.03,
  recent50RoiFloorJ: -0.08,

  // K：近期 50 輪中2有，但中3完全跟不上，代表上限太低
  minRoundsK: 12,
...
function calcStableScore({
  ...
  const roiPart = roiClamped * 90;
  const avgHitPart = avgHitDelta * 55;
  const recentRoiPart = recentRoiClamped * 65;
  const recentHitRatePart = hitRateClamped * 20;
  const roundsBonus = roundsClamped * 1.4;

  /**
   * 保留 hit 次數加分，但進一步降低 count 對舊策略的灌分效果。
   */
  const hitCountBonus =
    toNum(hit2, 0) * 1.5 +
    toNum(hit3, 0) * 7 +
  ...
  /**
   * 核心升級：
   * 讓 hit3_rate / recent_50_hit3_rate / hit4_rate 真正成為主權重
   */
  const qualityRateBonus =
    clamp(hit2_rate, 0, 1) * 40 +
    hit3RateClamped * 340 +
    hit4RateClamped * 700 +
    recentHit3RateClamped * 320 +
    recentHit4RateClamped * 520;
...
function shouldDisableStrategy(row = {}) {
  ...
  /**
   * 額外保護先做：
   * 有 hit4_rate 或近期 hit3 很明顯的策略，先保護，不要太早砍。
   */
