...
/* =========================
 * market learning
 * 寫進 strategy_pool.parameters.market_stats
 * ========================= */

function normalizeZoneCounts(value) {
  if (!Array.isArray(value)) return [0, 0, 0, 0];
  const arr = value.map((x) => toInt(x, 0)).slice(0, 4);
...
function normalizeMarketSummary(summary = {}) {
  ...
  return {
    ...
    zone_counts: normalizeZoneCounts(s.zone_counts)
  };
}

function extractMarketSummaryFromAny(input) {
...
function trimMarketBuckets(buckets = {}) {
...
function buildMarketBucketPatch({
  ...
  const marketStats = safeObject(parameters.market_stats);
  const buckets = safeObject(marketStats.buckets);

  const marketKey = buildMarketBucketKey(marketSummary);
  const prevBucket = safeObject(buckets[marketKey]);
  ...
  const prevRounds = toNum(prevBucket.total_rounds, 0);
  const prevHits = toNum(prevBucket.total_hits, 0);
  const prevCost = toNum(prevBucket.total_cost, 0);
  const prevReward = toNum(prevBucket.total_reward, 0);
  const prevProfit = toNum(prevBucket.total_profit, 0);

  const nextRounds = prevRounds + 1;
  const nextHits = prevHits + currentHit;
  const nextCost = prevCost + currentCost;
  const nextReward = prevReward + currentReward;
  const nextProfit = prevProfit + currentProfit;

  let hit0 = toNum(prevBucket.hit0, 0);
  let hit1 = toNum(prevBucket.hit1, 0);
  let hit2 = toNum(prevBucket.hit2, 0);
  let hit3 = toNum(prevBucket.hit3, 0);
  let hit4 = toNum(prevBucket.hit4, 0);
  ...
  const recentProfit = trimRecent(
    [...normalizeRecentArray(prevBucket.recent_profit), currentProfit],
    MARKET_BUCKET_RECENT_WINDOW
  );

  const recentBestHits = trimRecent(
    [...normalizeRecentArray(prevBucket.recent_best_hits), currentBestSingleHit],
    MARKET_BUCKET_RECENT_WINDOW
  );
  ...
  const nextBuckets = trimMarketBuckets({
    ...buckets,
    [marketKey]: nextBucket
  });
...
async function updateStrategyPoolMarketLearning({
  supabase,
  strategyKey,
  marketSummary,
  result,
...
export async function recordStrategyCompareResult({
  ...
  const marketSummary =
    extractMarketSummaryFromAny(marketSnapshot) ||
    extractMarketSummaryFromAny(compareResult) ||
    extractMarketSummaryFromAny(marketSignal);
  ...
  for (const result of results) {
    ...
    if (marketSummary) {
      marketLearningResult = await updateStrategyPoolMarketLearning({
        supabase,
        strategyKey,
        marketSummary,
        result,
    ...
    marketBucketUpdates.push(marketLearningResult);
    ...
    updated.push({
      ...
      market_key: marketLearningResult?.market_key || null
    });
  }
