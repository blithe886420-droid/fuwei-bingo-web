...
export async function recordStrategyCompareResult(compareResult) {
  ...
  // ✅ 修正第2點：同一策略在同一期可能出現多組（allocation > 1）
  // 若直接對每組都記一筆 recent_hits，同期多組會稀釋命中率
  // 正確做法：同一 draw_no 同一策略，只記一次 hit（取最高命中數）
  const drawNo = detail[0]?.draw_no || 0;
  const bestHitPerStrategy = new Map();
  ...
  for (const row of detail) {
    ...
    // ✅ 取這個策略這期所有組裡最高的命中數（修正同期多組稀釋問題）
    const hit = bestHitPerStrategy.get(key) ?? toNum(row?.hit, 0);
    ...
    // 取這個策略這期所有組裡最高的覆蓋命中數
    const coverageHit = bestCoveragePerStrategy.get(key) ?? toNum(row?.coverage_hit, 0);
    ...
    if (is3starMode) {
      // ✅ 修正二：三星淘汰條件補強
      // 原本只有「30期後 hit2=0 && hit3=0」，三星hit2理論14%，這條幾乎永遠不觸發
      // 補充以下三條實用淘汰條件：

      // 條件A：30期後 hit2=0 且 hit3=0（原有條件保留）
      const cond_A = payload.total_rounds >= 30 &&
        payload.hit2 === 0 && payload.hit3 === 0;

      // 條件B：50期後 hit2率低於3%（理論14%，低於3%屬嚴重異常）且ROI極差
      const cond_B = payload.total_rounds >= 50 &&
        payload.hit2_rate < 0.03 &&
        payload.recent_50_roi < -0.80;

      // 條件C：40期後近50期hit2率低於4%，且近50期hit3率為0，且ROI很差
