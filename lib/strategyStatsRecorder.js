...
export async function recordStrategyCompareResult(compareResult) {
  ...
  const isFormal3Star = compareResult?.mode === 'formal_3star' || compareResult?.star_mode === 3 || compareResult?.star_mode === '3';
  console.log('[strategyStatsRecorder] isFormal3Star:', isFormal3Star, 'mode:', compareResult?.mode, 'star_mode:', compareResult?.star_mode);
  if (!isFormal3Star) {
  ...
  // ✅ v11：改用 strategy_key 為統計主鍵（移除 v10 的前綴邏輯）
  // 同一 strategy_key 不論前綴（HOT|/COLD|/ZONE_FILL|）都合併統計
  const drawNo = detail[0]?.draw_no || 0;
  const bestHitPerStrategy = new Map(); // key = strategy_key
  const bestCoveragePerStrategy = new Map();
  for (const row of detail) {
    const k = String(row?.strategy_key || '').trim();
    if (!k) continue;
  ...
  // ✅ v11：每個 strategy_key 只處理一次
  const processedKeys = new Set();

  for (const row of detail) {
    // ✅ v11：主鍵改為 strategy_key（純 key，無前綴）
    const key = String(row?.strategy_key || '').trim();
    const nameKey = key; // strategy_name 統一等於 strategy_key
    if (!key) {
    ...
    const { error: upsertError } = await supabase
      .from(STRATEGY_STATS_TABLE)
      .upsert(payload, { onConflict: 'strategy_key' }); // ✅ v11：衝突鍵改為 strategy_key
