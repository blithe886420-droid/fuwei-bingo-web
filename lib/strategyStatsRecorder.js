...
export async function recordStrategyCompareResult(compareResult) {
  ...
  for (const row of compareResult.detail) {
    ...
    const recentHits = tailArray([...(Array.isArray(existing?.recent_hits) ? existing.recent_hits : []), hit], 50);
    const recentProfit = tailArray(
      [...(Array.isArray(existing?.recent_profit) ? existing.recent_profit : []), profit],
      50
    );
    ...
    const recent50Roi =
      recentProfit.length > 0
        ? recentProfit.reduce((sum, x) => sum + toNum(x, 0), 0) / (recentProfit.length * Math.max(cost, 1))
        : 0;
