...
function calcStableScore({
  ...
  const roiPart = roiClamped * 80;
  const avgHitPart = avgHitDelta * 45;
  const recentRoiPart = recentRoiClamped * 55;
  const recentHitRatePart = hitRateClamped * 12;
  const roundsBonus = roundsClamped * 1.1;

  /**
   * 降低純 count 對分數的影響，避免老策略天然灌分。
   */
  const hitCountBonus =
    toNum(hit2, 0) * 1.0 +
    toNum(hit3, 0) * 8 +
  ...
  const qualityRateBonus =
    ...
    hit3RateClamped * 420 +
    hit4RateClamped * 900 +
    recentHit3RateClamped * 420 +
    recentHit4RateClamped * 700;
