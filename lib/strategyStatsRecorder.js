...
function calcStableScore({
  ...
  // ✅ 修正二：加入覆蓋命中率作為評分依據
  // 覆蓋命中率高的策略，選號跟開獎熱區重疊多，長期中3機率更高
  // avg_coverage_hit 理論值是 6（24個覆蓋號碼 × 20/80），高於6代表選號比隨機更準
  const avgCoverageHit = toNum(row?.avg_coverage_hit, 0);
  const recentCoverageHitRate = toNum(row?.recent_coverage_hit_rate, 0);
  // 覆蓋命中超過理論值6才加分，低於5扣分
  const coveragePart = avgCoverageHit > 6 ? (avgCoverageHit - 6) * 40 :
                       avgCoverageHit < 5 ? (avgCoverageHit - 5) * 30 : 0;
  const recentCoveragePart = recentCoverageHitRate > 6 ? (recentCoverageHitRate - 6) * 30 : 0;
  ...
  return round4(
    roiPart +
      ...
      recentTruthPart +
      countPart +
      coveragePart +        // ✅ 覆蓋命中率貢獻
      recentCoveragePart +  // ✅ 近期覆蓋命中率貢獻
      honestyAdjustment
  );
}
