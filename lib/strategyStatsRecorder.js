...
export async function recordStrategyCompareResult(compareResult) {
  ...
  for (const row of detail) {
    ...
    // ✅ 傳入覆蓋命中率（修正之前 calcStableScore 裡用 row?.avg_coverage_hit 的 bug）
    // avg_coverage_hit 和 recent_coverage_hit_rate 在這裡才計算出來，傳入給 calcStableScore
    const score = calcStableScore({
      ...
      recent_50_hit4_rate,
      recent_hits,
      avg_coverage_hit,       // ✅ 修正：正確傳入
      recent_coverage_hit_rate // ✅ 修正：正確傳入
    });
