...
function shouldDisableStrategy(row = {}) {
  ...
  // ✅ 優化四：加入覆蓋率貢獻保護
  // 覆蓋率高的策略（avg_coverage_hit > 4.5）即使命中率不佳也保留，
  // 因為它幫助覆蓋其他策略沒有覆蓋到的號碼，整體中3機率更高
  const avgCoverageHit = toNum(row?.avg_coverage_hit, 0);
  const recentCoverageHitRate = toNum(row?.recent_coverage_hit_rate, 0);
  if (avgCoverageHit >= 4.5 || recentCoverageHitRate >= 4.0) {
    return { shouldDisable: false, reason: '' }; // 覆蓋率優秀，保留
  }
