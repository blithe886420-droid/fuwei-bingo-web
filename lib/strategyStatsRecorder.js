...
function buildEmptyPhaseBucket() {
  return {
    rounds: 0,
    total_hits: 0,
    total_cost: 0,
    ...
    recent_profit: [],
    recent_cost: [],
    recent_20_hit_rate: 0,
    recent_20_hit3_rate: 0,
    recent_20_hit4_rate: 0,
    recent_20_roi: 0
  };
}
...
function updatePhaseStatsJson(oldValue, marketPhase, hit, cost, reward, profit) {
  ...
  const bucket = normalizePhaseBucket(phaseStats[phase] || {});
  bucket.rounds += 1;
  ...
  phaseStats[phase] = normalizePhaseBucket(bucket);
  return phaseStats;
}
