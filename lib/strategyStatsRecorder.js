...
const DISABLE_RULES = {
  ...
  minRoundsB: 4,
  recent50RoiFloorB: -0.28,
  avgHitFloorB: 0.95,

  // C：近期 ROI 差 + 命中率差（命中2以上）
  minRoundsC: 6,
  recent50RoiFloorC: -0.42,
  hitRateFloorC: 0.12,
  ...
  minRoundsF: 3,
  avgHitFloorF: 0.75,
  recent50RoiFloorF: -0.48,

  // G：跑一段時間，但 hit3 能力太弱
  minRoundsG: 6,
  hit3RateFloorG: 0.06,
  recent50RoiFloorG: -0.12,
  avgHitFloorG: 1.1,
  ...
  minRoundsI: 5,
  recent50Hit3RateFloorI: 0.02,
  recent50RoiFloorI: -0.12,
  recent50HitRateFloorI: 0.14,

  // J：只會保2，拉不起中3，上限太低
  minRoundsJ: 8,
  hit2RateCeilJ: 0.28,
  hit3RateFloorJ: 0.05,
  recent50RoiFloorJ: -0.12,

  // K：近期中2還行，但 hit3 幾乎沒出現，且整體 ROI 仍負
  minRoundsK: 8,
  recent50HitRateFloorK: 0.16,
...
function calcStableScore({
  ...
  const roiPart = roiClamped * 30;
  const avgHitPart = avgHitDelta * 72;
  const recentRoiPart = recentRoiClamped * 18;
  const recentHitRatePart = hitRateClamped * 22;
  const roundsBonus = roundsClamped * 0.9;

  /**
   * 降低 ROI 對盤型切換時的壓制，強化 hit3 / hit4 爆發。
   */
  const hitCountBonus =
    toNum(hit2, 0) * 1.4 +
    toNum(hit3, 0) * 18 +
  ...
  /**
   * 命中3 / 命中4 為主，近期 hit3 / hit4 直接拉高。
   */
  const qualityRateBonus =
    clamp(hit2_rate, 0, 1) * 20 +
    hit3RateClamped * 680 +
    hit4RateClamped * 1400 +
    recentHit3RateClamped * 760 +
    recentHit4RateClamped * 1080;
...
function shouldDisableStrategy(row = {}) {
  ...
  const recent50Hit4Rate = toNum(row?.recent_50_hit4_rate);
  ...
  if (
    hit4Rate >= 0.01 ||
    recent50Hit4Rate >= 0.02 ||
    recent50Hit3Rate >= 0.06 ||
    hit3Rate >= 0.08 ||
    hit4 >= 1 ||
    (avgHit >= 1.35 && totalRounds >= 4)
  ) {
    return {
      shouldDisable: false,
      reason: ''
