...
export function buildRecentMarketSignalSnapshot(rows = [], drawNumbersCol = 'numbers') {
  ...
  // 四種盤相判斷：
  // continuation：連號多 + 熱區集中 → 追熱號效果最好
  // bias：區段偏移大 → 追偏熱區段
  // rotation：熱區分散 + 連號少 → 均衡選號
  // chaos：最新一期熱號很少 + 無連號 → 完全分散
  // ✅ 修復：hotConcentration 永遠是 4-5（hot5有5個，hot10有10個，至少重疊4個），沒有鑑別力
  // 改用更精準的條件：
  // continuation：真的有連號（streakScore高）才判斷
  // bias：最近3期某區段特別集中（zoneBias高）
  // chaos：最新一期熱號很少出現（latestHot10Hit低）且無連號
  // rotation：其他情況
  // ✅ fix：調低門檻，讓步驟六真正有鑑別力
  // 原門檻 streakScore>=10, zoneBias>=20 在實際數據下幾乎永遠是 rotation
  // 根據實際 log 數據（streak=0, zoneBias=3~16, latestHot10=20）重新校準
  let market_phase;
  if (streakScore >= 3) {
    market_phase = 'continuation';  // 有一點連號就算 continuation
  } else if (zoneBias >= 8) {
    market_phase = 'bias';          // 區段稍微偏就算 bias（原20→8）
  } else if (latestHot10Hit <= 4 && streakScore === 0) {
    market_phase = 'chaos';         // 熱號少且完全無連號
  } else {
    market_phase = 'rotation';      // 真正均衡輪動
  }

  console.log(`[market_phase] streak=${streakScore} hotConc=${hotConcentration} zoneBias=${zoneBias} latestHot10=${latestHot10Hit} → ${market_phase}`);
