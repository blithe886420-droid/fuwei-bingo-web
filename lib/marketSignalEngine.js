...
export function buildRecentMarketSignalSnapshot(rows = [], drawNumbersCol = 'numbers') {
  ...
  // ✅ 修復：market_phase 原本幾乎永遠是 rotation
  // 原因：rotationScore 包含 recentUniqueCount（接近80）和 80-hot10Overlap（也很大）
  // 導致 continuationScore 永遠輸，所有依賴 market_phase 的選號邏輯都走 rotation 那條路
  // 修復：改用更精準的四種盤相判斷（continuation/bias/rotation/chaos）

  // 連號強度（越高代表盤面越有延續性）
  const streakScore = streak2Count * 2 + streak3Count * 8 + (streaks.streak4?.length || 0) * 20;

  // 熱區集中度（hot5 和 hot10 重疊越多代表熱區越穩定）
  const hot5Set = new Set(Array.isArray(hot5?.numbers) ? hot5.numbers : []);
  const hot10Nums = Array.isArray(hot10?.numbers) ? hot10.numbers : [];
  const hotConcentration = hot10Nums.filter(n => hot5Set.has(n)).length;

  // 最新一期跟熱區的重疊（最新一期開出多少熱號）
  const latestNums = latestSignal?.numbers || [];
  const latestHot10Hit = latestNums.filter(n => hot10Set.has(n)).length;

  // ✅ v3：熱號穩定度改用「高頻熱號」：最近5期出現>=2次的號碼才算真正的熱號
  // 原本 hot5.numbers 包含最近5期所有出現過的號碼（最多80顆），鑑別力很低
  // 改用出現>=2次的號碼，通常只有10~20顆，才是真正的高頻熱號
  const hot5Nums = Array.isArray(hot5?.numbers) ? hot5.numbers : [];
  const hot5Items = Array.isArray(hot5?.items) ? hot5.items : [];
  const trueHotNums = new Set(hot5Items.filter(item => item.count >= 2).map(item => item.num));
  const trueHotCount = trueHotNums.size;
  const latestTrueHotHit = latestNums.filter(n => trueHotNums.has(n)).length;
  const hot5hot10Overlap = hot5Nums.filter(n => hot10Set.has(n)).length;

  // 最近5期開獎號碼的熱號命中率（連續命中越高代表盤面越穩定）
  const recent5HotHits = safeRows.slice(0, 5).map(row => {
    const nums = parseNumbersFromValue(row?.[drawNumbersCol]);
    return nums.filter(n => hot10Set.has(n)).length;
  });
  const avgRecent5HotHit = recent5HotHits.length > 0
    ? recent5HotHits.reduce((a, b) => a + b, 0) / recent5HotHits.length
    : 0;

  // 區段偏移（最近3期某個區段特別集中）
  const recentZoneDist = [0, 0, 0, 0];
  safeRows.slice(0, 3).forEach(row => {
    parseNumbersFromValue(row?.[drawNumbersCol]).forEach(n => {
  ...
  const zoneBias = maxZone - minZone;

  // ✅ v2：六種盤相判斷（從4種細化為6種）
  // 原本只有 4 種，bias 和 rotation 各佔約 50% 但內部差異很大
  // 新增：hot_bias（bias盤中熱號穩定）、hot_rotation（rotation盤中熱號活躍）
  // continuation 門檻從 streakScore>=10 改為同時考慮 streak + 熱號穩定度，讓它更容易觸發
  let market_phase;

  if (streakScore >= 5 && hot5hot10Overlap >= 4) {
    // continuation：有連號 + 熱號穩定
    market_phase = 'continuation';
  } else if (streakScore >= 3 && avgRecent5HotHit >= 8) {
    // continuation_weak：有輕微連號 + 近期熱號命中穩定
    market_phase = 'continuation';
  } else if (zoneBias >= 12 && latestHot10Hit >= 7) {
    // hot_bias：區段偏移 + 最新一期熱號多
    market_phase = 'hot_bias';
  } else if (zoneBias >= 8) {
    // bias：區段偏移
    market_phase = 'bias';
  } else if (latestTrueHotHit >= 10 && avgRecent5HotHit >= 12) {
    // ✅ v3：hot_streak 門檻大幅提高，用真正高頻熱號（出現>=2次）計算
    // latestTrueHotHit >= 10：最新一期有10顆真正高頻熱號
    // avgRecent5HotHit >= 12：最近5期平均命中12顆熱號，持續熱
    market_phase = 'hot_streak';
  } else if (latestHot10Hit <= 4 && streakScore === 0) {
    // chaos：最新一期熱號極少 + 完全無連號
    market_phase = 'chaos';
  } else {
    // rotation：正常輪動
    ...
    console.log(`[market_phase] streak=${streakScore} hotConc=${hotConcentration} zoneBias=${zoneBias} latestHot10=${latestHot10Hit} avgHot5=${avgRecent5HotHit.toFixed(1)} overlap=${hot5hot10Overlap} → ${market_phase}`);
  ...
  const baseSnapshot = {
    ...
    // ✅ v3：新增高頻熱號相關欄位
    true_hot_nums: [...trueHotNums],      // 出現>=2次的真正高頻熱號
    true_hot_count: trueHotCount,          // 高頻熱號數量
    latest_true_hot_hit: latestTrueHotHit  // 最新一期命中幾顆高頻熱號
  };
