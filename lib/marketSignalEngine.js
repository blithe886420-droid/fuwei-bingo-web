...
export function buildRecentMarketSignalSnapshot(rows = [], drawNumbersCol = 'numbers') {
  ...
  // 熱號穩定度：hot5 和 hot10 的重疊（穩定度越高代表熱號越可靠）
  ...
  if (streakScore >= 5 && hot5hot10Overlap >= 4) {
    // continuation：有連號 + 熱號穩定 → 追熱號和連號效果最好
    ...
    // hot_bias：區段偏移 + 最新一期熱號多 → 熱區偏移型，追偏熱區段
    market_phase = 'hot_bias';
  } else if (zoneBias >= 8) {
    // bias：區段偏移 → 某區段偏熱
    market_phase = 'bias';
  } else if (latestHot10Hit >= 9 && avgRecent5HotHit >= 8) {
    // hot_streak：熱號持續大量命中 → 熱號高度集中期
    market_phase = 'hot_streak';
  } else if (latestHot10Hit <= 4 && streakScore === 0) {
    // chaos：最新一期熱號極少 + 完全無連號 → 完全分散
    market_phase = 'chaos';
  } else {
    // rotation：正常輪動
