...
export async function maybeRunStrategyEvolution() {
  ...
  const worstStrategies = actualEliminationCount > 0
    ? maturedCandidates.slice(-actualEliminationCount)
    : [];
  ...
  if (worstStrategies.length) {
    ...
    historyRows.push(
      ...worstStrategies.map((row) => ({
        ...
        event_type: 'disabled',
        note: 'auto disabled by v4.3 evolution engine',
        payload: {
          currentDrawNo,
  ...
  if (createdStrategies.length) {
    ...
    historyRows.push(
      ...createdStrategies.map((row) => ({
        strategy_key: row.strategy_key,
        event_type: 'created',
        note: 'auto created by v4.3 evolution engine',
        payload: {
          currentDrawNo,
