...
export async function maybeRunStrategyEvolution() {
  ...
  if (!pool || pool.length === 0) {
    return { skipped: true, reason: 'no active strategies' };
  }
  ...
  (stats || []).forEach(s => statMap.set(s.strategy_key, s));

  const ranked = pool.map(p => {

    const stat = statMap.get(p.strategy_key) || {};

    return {
      ...p,
      ...stat,
      score: calculateEvolutionScore(stat)
    };

  }).sort((a,b)=>b.score-a.score);

  const existingKeys = new Set(pool.map(p=>p.strategy_key));
  ...
  const currentDrawNo = drawRow ? Number(drawRow.draw_no) : 0;
  ...
  for (let i=0;i<2;i++){
    ...
    if (shouldUseExploration()) {

      s = buildExplorationStrategy({
        existingKeys,
        currentDrawNo,
        ranked,
        incubationDraws: DEFAULT_INCUBATION_DRAWS
      });

    } else {

      s = buildCrossoverStrategy({
        ranked,
        existingKeys,
        currentDrawNo,
        incubationDraws: DEFAULT_INCUBATION_DRAWS
