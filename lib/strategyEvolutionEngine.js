...
function getStrategyGenes(strategy) {
  return unique([strategy?.gene_a, strategy?.gene_b]);
}
...
function buildExplorationStrategy({ existingKeys, currentDrawNo, ranked, incubationDraws }) {
  ...
  const geneB = randomPick(BASE_GENE_POOL.filter((g) => g !== geneA)) || 'mix';
...
export async function maybeRunStrategyEvolution() {
  ...
  const eliminationCount = toNum(configMap.strategy_elimination_count, DEFAULT_ELIMINATION_COUNT);
  const protectTopN = toNum(configMap.strategy_protect_top_n, DEFAULT_PROTECT_TOP_N);
  ...
  if (worstStrategies.length) {
    ...
    historyRows.push(
      ...worstStrategies.map((row) => ({
        ...
        event_type: 'disabled',
        note: 'auto disabled by v3.7 evolution engine',
        payload: {
          currentDrawNo,
  ...
  if (createdStrategies.length) {
    ...
    historyRows.push(
      ...createdStrategies.map((row) => ({
        strategy_key: row.strategy_key,
        event_type: 'created',
        note: 'auto created by v3.7 evolution engine',
        payload: {
          currentDrawNo,
