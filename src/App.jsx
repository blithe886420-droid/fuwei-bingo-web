...
const PARALLEL_UI = [
  ...
  { key: 'shadow_delay_diverse', label: '挑戰1・延遲分散', short: '挑1' },
...
const REFERENCE_LANE_FALLBACK = {
  ...
  shadow_neighbor: ['shadow_neighbor'],
  shadow_freq: ['shadow_freq'],
  shadow_cross: ['shadow_cross'],
  shadow_neighbor_balanced: ['shadow_neighbor_balanced', 'shadow_consec', 'lab_neighbor_balanced'],
};
...
const SELECTION_STRATEGY_LABEL = {
  ...
  board_ef_ca:{ text: '🎯 盤面適配', color: '#7C2D12', bg: '#FFEDD5' },
  ...
  consec_cover_t120:{ text: '🌐 覆蓋加強', color: '#0369A1', bg: '#E0F2FE' },
  consec_fast_t60:{ text: '⚡ 快速反應', color: '#7C3AED', bg: '#F5F3FF' },
  ...
  anti_last_t120:{ text: '🚫 避開上期', color: '#B45309', bg: '#FEF3C7' },
...
function StatsPage({ audit }) {
  ...
  if (!audit?.ok) {
    return (
      <div style={S.page}>
        <Card title="📊 V0808 固定窗口" icon="">
          <div style={S.empty}>固定成績表尚未就緒，請先執行 V0808 SQL，部署後會自動累積。</div>
        </Card>
      </div>
    );
  }

  const displayKeys = laneOrder.filter(k => !String(k).startsWith('lab_'));
  const labKeys = laneOrder.filter(k => String(k).startsWith('lab_'));

  return (
    <div style={S.page}>
      <div style={{ ...S.card, border: `2px solid ${C.gold}` }}>
        <div style={{ fontSize: 15, fontWeight: 900, color: C.gold }}>V0808 顯示六路（無實驗席）</div>
      ...
      <div style={{ fontSize: 12, fontWeight: 800, color: C.text, margin: '4px 0 8px' }}>顯示六路</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8, marginBottom: 12 }}>
        {displayKeys.map(laneKey => {
      ...
      {labKeys.length > 0 && (
        <>
          <div style={{ fontSize: 12, fontWeight: 800, color: C.text, margin: '4px 0 8px' }}>實驗席（不顯示於快速／近期）</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8, marginBottom: 12 }}>
            {labKeys.map(laneKey => {
              const lane = lanes?.[laneKey] || {};
              const s = lane?.summary || {};
              ...
              const waiting = !g.sample_ready;
              return (
                <div key={laneKey} style={{ ...S.card, marginBottom: 0, padding: 11, border: `1px dashed ${C.border}`, background: '#F8FAFC' }}>
                  <div style={{ fontSize: 12, fontWeight: 900, minHeight: 34 }}>{lane.label || laneKey}</div>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 5, marginTop: 4 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 720, fontSize: 10 }}>
              <thead>
                <tr>
                  ...
                  {laneOrder.map(key => <th key={key} style={{ padding: 6, borderBottom: `1px solid ${C.border}` }}>{parallelShort(key)}</th>)}
                </tr>
              </thead>
              <tbody>
                {hourly.map(row => (
    ...
    <div style={{ padding: '10px 12px 4px', background: '#F8FAFC' }}>
      ...
      {audit?.hour_playbook?.follow_label ? (
        <div style={{
          ...
          此刻劇本（{audit.hour_playbook.band_label || '時段'} {audit.hour_playbook.now_hour}:00）：
          優先跟 <b>{parallelShort(audit.hour_playbook.follow_key)}</b>
          （{audit.hour_playbook.follow_label}）
          <span style={{ fontWeight: 600, color: '#166534' }}> — {audit.hour_playbook.note}</span>
        </div>
      ) : null}
      ...
      {toNum(activeSummary.total, 0) > 0 && !String(activeKey).startsWith('lab_') && (
        <div style={{ fontSize: 11, color: C.textSub, marginTop: 7, textAlign: 'center', lineHeight: 1.6 }}>
          ...
          <div>
            E場 合格 {toNum(eSummary.qualified, 0)}/{toNum(eSummary.total, 0)}
            （{fmtPercent(eSummary.qualified_rate)}）｜
            F場 合格 {toNum(fSummary.qualified, 0)}/{toNum(fSummary.total, 0)}
            （{fmtPercent(fSummary.qualified_rate)}）
          </div>
        </div>
      )}
    </div>
